			    ...
			    u32 size, u8 *efuse_map)
{
	...
	if (!efuse_map) {
		PLATFORM_MSG_PRINT(halmac_adapter->driver_adapter,
				   HALMAC_MSG_EFUSE, HALMAC_DBG_ERR,
			...
			u32 dest, u32 code_size)
{
	...
	if (halmac_check_fw_chksum_88xx(halmac_adapter, dest) !=
	    HALMAC_RET_SUCCESS) {
		PLATFORM_MSG_PRINT(halmac_adapter, HALMAC_MSG_INIT,
				   HALMAC_DBG_ERR,
		       ...
		       u32 dest, u32 length, u8 first)
{
	...
	while (HALMAC_REG_READ_32(halmac_adapter, REG_DDMA_CH0CTRL) &
		...
		if (counter == 0) {
			PLATFORM_MSG_PRINT(
				driver_adapter, HALMAC_MSG_FW, HALMAC_DBG_ERR,
	...
	while (HALMAC_REG_READ_16(halmac_adapter, REG_MCUFW_CTRL) != 0xC078) {
		if (counter == 0) {
			...
			if ((HALMAC_REG_READ_32(halmac_adapter, REG_FW_DBG7) &
				...
				PLATFORM_MSG_PRINT(
					driver_adapter, HALMAC_MSG_INIT,
					HALMAC_DBG_ERR, "Key fail\n");
			return HALMAC_RET_DLFW_FAIL;
		}
		counter--;
		udelay(50);
		...
		do {
			...
			if (halmac_adapter->halmac_interface ==
				...
				PLATFORM_MSG_PRINT(
					...
					"[ERR]Pwr cmd value : %X!!\n",
					sub_seq_cmd->value);
				PLATFORM_MSG_PRINT(driver_adapter,
						   HALMAC_MSG_PWR,
						   HALMAC_DBG_ERR,
						   "[ERR]Pwr cmd msk : %X!!\n",
						   sub_seq_cmd->msk);
			   ...
			   u8 *hal_h2c_hdr, u16 *seq, bool ack)
{
	...
	*seq = halmac_adapter->h2c_packet_seq;
	...
	mutex_unlock(&halmac_adapter->h2c_seq_mutex);
			      ...
			      u8 *c2h_buf, u32 c2h_size)
{
	...
	PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_H2C, HALMAC_DBG_TRACE,
			   "[TRACE]Seq num : h2c -> %d c2h -> %d\n",
			   halmac_adapter->halmac_state.iqk_set.seq_num,
			   h2c_seq);
				       ...
				       u32 *halmac_offset)
{
	...
	default:
		*halmac_offset = 0xFFFFFFFF;
		PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_INIT,
				   HALMAC_DBG_ERR, "Unknown base address!!\n");
...
halmac_verify_io_88xx(struct halmac_adapter *halmac_adapter)
{
	...
	if (halmac_adapter->halmac_interface == HALMAC_INTERFACE_SDIO) {
		...
		if (value32 != wvalue32) {
			PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_INIT,
					   HALMAC_DBG_ERR, "cmd53 w fail\n");
			ret_status = HALMAC_RET_PLATFORM_API_INCORRECT;
		} else {
			PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_INIT,
					   HALMAC_DBG_TRACE, "cmd53 w ok\n");
		}
		...
		if (value32 != wvalue32) {
			PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_INIT,
					   HALMAC_DBG_ERR, "reg rw\n");
			ret_status = HALMAC_RET_PLATFORM_API_INCORRECT;
		} else {
			PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_INIT,
					   HALMAC_DBG_TRACE, "reg rw ok\n");
		}
	}
			   ...
			   enum hal_intf_phy intf_phy)
{
	...
	for (curr_phy_para = intf_phy_para;; curr_phy_para++) {
		...
		if (ip_sel == HALMAC_IP_SEL_MAC) {
			...
			if (result != HALMAC_RET_SUCCESS)
				PLATFORM_MSG_PRINT(
					driver_adapter, HALMAC_MSG_MDIO,
					HALMAC_DBG_ERR,
			...
			u32 data)
{
	...
	PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_DBI, HALMAC_DBG_TRACE,
			   "WriteAddr = %x\n", write_addr);
	...
	if (tmp_u1b) {
		PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_DBI,
				   HALMAC_DBG_ERR, "DBI write fail!\n");
		return HALMAC_RET_FAIL;
	} else {
		return HALMAC_RET_SUCCESS;
	}
...
u32 halmac_dbi_read32_88xx(struct halmac_adapter *halmac_adapter, u16 addr)
{
	...
	if (tmp_u1b) {
		ret = 0xFFFF;
		PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_DBI,
				   HALMAC_DBG_ERR, "DBI read fail!\n");
	} else {
		ret = HALMAC_REG_READ_32(halmac_adapter, REG_DBI_RDATA_V1);
		PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_DBI,
				   HALMAC_DBG_TRACE, "Read Value = %x\n", ret);
	}
		       ...
		       u8 speed)
{
	...
	if (speed == HAL_INTF_PHY_PCIE_GEN1) {
		...
		PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_MDIO,
				   HALMAC_DBG_ERR, "Error Speed !\n");
	}
	...
	if (tmp_u1b) {
		PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_MDIO,
				   HALMAC_DBG_ERR, "MDIO write fail!\n");
		return HALMAC_RET_FAIL;
	} else {
		return HALMAC_RET_SUCCESS;
	}
			 ...
			 u16 data, u8 speed)
{
	...
	if (speed == HAL_INTF_PHY_USB3) {
		...
		PLATFORM_MSG_PRINT(driver_adapter, HALMAC_MSG_USB,
				   HALMAC_DBG_ERR, "[ERR]Error USB Speed !\n");
		return HALMAC_RET_NOT_SUPPORT;
	}
