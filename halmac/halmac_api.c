		    ...
		    struct halmac_api **pp_halmac_api)
{
	...
	if (status != HALMAC_RET_SUCCESS)
		return status;
	halmac_platform_api->MSG_PRINT(driver_adapter, HALMAC_MSG_INIT,
				       HALMAC_DBG_ERR, HALMAC_SVN_VER "\n");
	halmac_platform_api->MSG_PRINT(
	...
	*pp_halmac_adapter = halmac_adapter;
	...
	mutex_init(&halmac_adapter->efuse_mutex);
	mutex_init(&halmac_adapter->h2c_seq_mutex);
...
halmac_deinit_adapter(struct halmac_adapter *halmac_adapter)
{
	...
	mutex_destroy(&halmac_adapter->efuse_mutex);
	mutex_destroy(&halmac_adapter->h2c_seq_mutex);
			  ...
			  struct halmac_platform_api *halmac_platform_api)
{
	...
	if (!halmac_platform_api->MSG_PRINT)
		return HALMAC_RET_PLATFORM_API_NULL;
