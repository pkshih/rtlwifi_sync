...
void rtl8822be_set_fw_pwrmode_cmd(struct ieee80211_hw *hw, u8 mode)
{
	...
	rtlpriv->btcoexist.btc_ops->btc_record_pwr_mode(
		rtlpriv, u1_h2c_set_pwrmode, H2C_8822B_PWEMODE_LENGTH);
				       ...
				       u8 *c2h_content_buf)
{
	...
	default:
		RT_TRACE(rtlpriv, COMP_FW, DBG_TRACE,
			 "[C2H], Unknown packet!! sub_cmd_id(%#X)!\n",
			 c2h_sub_cmd_id);
	}
}
...
void rtl8822be_c2h_packet_handler(struct ieee80211_hw *hw, u8 *buffer, u8 len)
{
	...
	case 0xFF:
		...
		if (!halmac_ops->halmac_c2h_handle(rtlpriv, buffer - 24,
						   len + 24))
			break;	/* handled by halmac */
		/* no break, go through default */
	default:
