...
static bool _rtl8822be_check_fw_read_last_h2c(struct ieee80211_hw *hw, u8 boxnum)
{
				      ...
				      u32 cmd_len, u8 *cmdbuffer)
{
	...
	while (!bwrite_success) {
		...
		if (u1b_tmp == 0xea) {
			if (rtl_read_byte(rtlpriv, REG_TXDMA_STATUS_8822B) == 0xea ||
			    rtl_read_byte(rtlpriv, REG_TXPKT_EMPTY_8822B) == 0xea)
				rtl_write_byte(rtlpriv, REG_SYS_CFG1_8822B + 3, 0xff);
		...
		while (!isfw_read) {
			...
			udelay(10);
			isfw_read = _rtl8822be_check_fw_read_last_h2c(hw,
								boxnum);
...
void rtl8822be_fill_h2c_cmd(struct ieee80211_hw *hw,
...
void rtl8822be_set_fw_pwrmode_cmd(struct ieee80211_hw *hw, u8 mode)
{
	...
	rtl8822be_fill_h2c_cmd(hw, H2C_8822B_SETPWRMODE, H2C_8822B_PWEMODE_LENGTH,
			     u1_h2c_set_pwrmode);
...
void rtl8822be_set_fw_media_status_rpt_cmd(struct ieee80211_hw *hw, u8 mstatus)
{
	...
	rtl8822be_fill_h2c_cmd(hw, H2C_8822B_MSRRPT, 3, parm);
}
				     ...
				     struct sk_buff *skb, u8 hw_queue)
{
	...
	rtlpriv->cfg->ops->fill_tx_special_desc(hw, (u8 *)pdesc, (u8 *)pbd_desc, skb, hw_queue);
...
bool rtl8822b_halmac_cb_write_data_rsvd_page(struct rtl_priv *rtlpriv, u8 *buf, u32 size)
{
...
bool rtl8822b_halmac_cb_write_data_h2c(struct rtl_priv *rtlpriv, u8 *buf, u32 size)
{
	struct sk_buff *skb = NULL;
...
void rtl8822be_set_fw_rsvdpagepkt(struct ieee80211_hw *hw, bool b_dl_finished)
{
	...
	memset(u1RsvdPageLoc, 0, sizeof(u1RsvdPageLoc));
...
static void rtl8822be_set_p2p_ctw_period_cmd(struct ieee80211_hw *hw, u8 ctwindow)
{
	u8 u1_ctwindow_period[1] = {ctwindow};

	rtl8822be_fill_h2c_cmd(hw, H2C_8822B_P2P_PS_CTW_CMD, 1, u1_ctwindow_period);
...
#if 0
static void _rtl8822be_c2h_ra_report_handler(struct ieee80211_hw *hw,
				    u8 *cmd_buf, u8 cmd_len)
{
	...
	rtl8822be_dm_dynamic_arfb_select(hw, rate, collision_state);
#endif
}
#endif
				  ...
				  u8 c2h_cmd_len, u8 *tmp_buf)
{
	...
	case C2H_8822B_BT_MP:
		...
		break;
#if 0
	case C2H_8822B_RA_RPT:
		_rtl8822be_c2h_ra_report_handler(hw, tmp_buf, c2h_cmd_len);
		break;
#endif
	default:
		if (!rtlpriv->phydm.ops->phydm_c2h_content_parsing(rtlpriv, c2h_cmd_id,
								   c2h_cmd_len, tmp_buf))
			break;
...
void rtl8822be_c2h_packet_handler(struct ieee80211_hw *hw, u8 *buffer, u8 len)
{
	...
	case 0xFF:
		...
		rtlpriv->halmac.ops->halmac_c2h_handle(rtlpriv, buffer - 24, len + 24);
		break;
	default:
		rtl8822be_c2h_content_parsing(hw, c2h_cmd_id, c2h_cmd_len, tmp_buf);
		break;
	}
}
