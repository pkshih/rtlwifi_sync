...
static void _rtl8822be_set_bcn_ctrl_reg(struct ieee80211_hw *hw,
				      u8 set_bits, u8 clear_bits)
{
	...
	rtl_write_byte(rtlpriv, REG_BCN_CTRL_8822B, (u8) rtlpci->reg_bcn_ctrl_val);
}
...
static void _rtl8822be_set_fw_clock_on(struct ieee80211_hw *hw,
...
static void _rtl8822be_download_rsvd_page(struct ieee80211_hw *hw)
{
	...
	rtl_write_byte(rtlpriv, REG_FWHW_TXQ_CTRL_8822B + 2, tmp_reg422 & (~BIT(6)));
	...
	do {
		...
		bcnvalid_reg = rtl_read_byte(rtlpriv, REG_FIFOPAGE_CTRL_2_8822B + 1);
		bcnvalid_reg = bcnvalid_reg | BIT(7);
		rtl_write_byte(rtlpriv, REG_FIFOPAGE_CTRL_2_8822B + 1, bcnvalid_reg);
	...
	if (!(BIT(7) & bcnvalid_reg))
		RT_TRACE(rtlpriv, COMP_INIT, DBG_WARNING, "Download RSVD page failed!\n");
	...
	if (b_recover)
		rtl_write_byte(rtlpriv, REG_FWHW_TXQ_CTRL_8822B + 2, tmp_reg422);
...
void rtl8822be_set_hw_reg(struct ieee80211_hw *hw, u8 variable, u8 *val)
{
	...
	case HW_VAR_BASIC_RATE:{
		...
		rtl_write_byte(rtlpriv, REG_RRSR_8822B + 1, (b_rate_cfg >> 8) & 0xff);
		}
		break;
	case HW_VAR_BSSID:
	...
	case HW_VAR_SIFS:
		...
		if (!mac->ht_enable)
			rtl_write_word(rtlpriv, REG_RESP_SIFS_OFDM_8822B, 0x0e0e);
		else
	...
	case HW_VAR_AMPDU_FACTOR:{
			...
			rtl_write_dword(rtlpriv,
				REG_AMPDU_MAX_LENGTH_8822B, ampdu_len);
		}
		break;
	...
	case HW_VAR_DUAL_TSF_RST:
		rtl_write_byte(rtlpriv, REG_DUAL_TSF_RST_8822B, (BIT(0) | BIT(1)));
		break;
	...
	case HW_VAR_SET_RPWM:
		...
		if (rpwm_val & BIT(7)) {
			rtl_write_byte(rtlpriv, REG_PCIE_HRPWM_8822B, (*(u8 *) val));
		} else {
...
#if 0	/* by halmac */
static bool _rtl8822be_llt_table_init(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	u8 txpktbuf_bndy;
	u8 u8tmp, testcnt = 0;
	...
	rtl_write_dword(rtlpriv, REG_RQPN_8822B, 0x80E60808);

	rtl_write_byte(rtlpriv, REG_TRXFF_BNDY_8822B, txpktbuf_bndy);
	rtl_write_word(rtlpriv, REG_TRXFF_BNDY_8822B + 2, 0x3d00 - 1);

	rtl_write_byte(rtlpriv, REG_DWBCN0_CTRL_8822B + 1, txpktbuf_bndy);
	rtl_write_byte(rtlpriv, REG_DWBCN1_CTRL_8822B + 1, txpktbuf_bndy);

	rtl_write_byte(rtlpriv, REG_BCNQ_BDNY_8822B, txpktbuf_bndy);
	rtl_write_byte(rtlpriv, REG_BCNQ1_BDNY_8822B, txpktbuf_bndy);

	rtl_write_byte(rtlpriv, REG_MGQ_BDNY_8822B, txpktbuf_bndy);
	rtl_write_byte(rtlpriv, 0x45D, txpktbuf_bndy);

	rtl_write_byte(rtlpriv, REG_PBP_8822B, 0x31);
	rtl_write_byte(rtlpriv, REG_RX_DRVINFO_SZ_8822B, 0x4);

	u8tmp = rtl_read_byte(rtlpriv, REG_AUTO_LLT_8822B + 2);
	rtl_write_byte(rtlpriv, REG_AUTO_LLT_8822B + 2, u8tmp | BIT(0));
...
static void _rtl8822be_gen_refresh_led_state(struct ieee80211_hw *hw)
{
	...
	if (ppsc->rfoff_reason == RF_CHANGE_BY_IPS)
		rtl8822be_sw_led_on(hw, pLed0);
	else if (ppsc->rfoff_reason == RF_CHANGE_BY_INIT)
		rtl8822be_sw_led_on(hw, pLed0);
	else
		rtl8822be_sw_led_off(hw, pLed0);
}
...
static bool _rtl8822be_init_trxbd(struct ieee80211_hw *hw)
{
...
#if 0
	rtl_write_byte(rtlpriv, REG_RSV_CTRL_8822B, 0x0);

	dwordtmp = rtl_read_dword(rtlpriv, REG_SYS_CFG1_8822B);
	if (dwordtmp & BIT(24)) {
	...
	bytetmp = rtl_read_byte(rtlpriv, REG_AFE_CTRL2_8822B);
	bytetmp &= 0xfb;
	rtl_write_byte(rtlpriv, REG_AFE_CTRL2_8822B, bytetmp);

	dwordtmp = rtl_read_dword(rtlpriv, REG_AFE_CTRL4_8822B);
	dwordtmp &= 0xfffffc7f;
	rtl_write_dword(rtlpriv, REG_AFE_CTRL4_8822B, dwordtmp);

	/* 2. 8822B AFE parameter
	...
	if (!rtl_hal_pwrseqcmdparsing(rtlpriv, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
		PWR_INTF_PCI_MSK, Rtl8822B_NIC_ENABLE_FLOW)) {
	...
	bytetmp = rtl_read_byte(rtlpriv, REG_CR_8822B);
	bytetmp = 0xff;
	rtl_write_byte(rtlpriv, REG_CR_8822B, bytetmp);
	mdelay(2);
	bytetmp = 0x7f;
	rtl_write_byte(rtlpriv, REG_HWSEQ_CTRL_8822B, bytetmp);
	mdelay(2);

	/* Add for wakeup online */
	bytetmp = rtl_read_byte(rtlpriv, REG_SYS_CLKR_8822B);
	rtl_write_byte(rtlpriv, REG_SYS_CLKR_8822B, bytetmp | BIT(3));
	bytetmp = rtl_read_byte(rtlpriv, REG_GPIO_MUXCFG_8822B + 1);
	rtl_write_byte(rtlpriv, REG_GPIO_MUXCFG_8822B + 1, bytetmp & (~BIT(4)));
	/* Release MAC IO register reset */
	rtl_write_word(rtlpriv, REG_CR_8822B, 0x2ff);

	if (!rtlhal->mac_func_enable) {
		if (_rtl8822be_llt_table_init(hw) == false) {
			RT_TRACE(rtlpriv, COMP_INIT, DBG_LOUD,
				 "LLT table init fail\n");
			return false;
		}
	...
	rtl_write_dword(rtlpriv, REG_HISR0_8822B, 0xffffffff);
	rtl_write_dword(rtlpriv, REG_HISR1_8822B, 0xffffffff);
	rtl_write_dword(rtlpriv, REG_HISR3_8822B, 0xffffffff);

	wordtmp = rtl_read_word(rtlpriv, REG_TRXDMA_CTRL_8822B);
	wordtmp &= 0xf;
	wordtmp |= 0xF5B1;
	rtl_write_word(rtlpriv, REG_TRXDMA_CTRL_8822B, wordtmp);
	/* Reported Tx status from HW for rate adaptive.*/
	rtl_write_byte(rtlpriv, REG_FWHW_TXQ_CTRL_8822B + 1, 0x1F);

	/* Set RCR register */
	rtl_write_dword(rtlpriv, REG_RCR_8822B, rtlpci->receive_config);
	rtl_write_word(rtlpriv, REG_RXFLTMAP2_8822B, 0xffff);

	/* Set TCR register */
	rtl_write_dword(rtlpriv, REG_TCR_8822B, rtlpci->transmit_config);
#endif

#if (DMA_IS_64BIT == 1)
	/* Set TX/RX descriptor physical address -- HI part */
	rtl_write_dword(rtlpriv, REG_H2CQ_TXBD_DESA_8822B + 4,
			((u64) rtlpci->tx_ring[H2C_QUEUE].buffer_desc_dma) >> 32);
	rtl_write_dword(rtlpriv, REG_BCNQ_TXBD_DESA_8822B + 4,
			((u64) rtlpci->tx_ring[BEACON_QUEUE].buffer_desc_dma) >> 32);
	...
	rtl_write_dword(rtlpriv, REG_H2CQ_CSR_8822B, (dwordtmp | BIT(8) | BIT(16)));
	...
	rtl_write_word(rtlpriv, REG_H2CQ_TXBD_NUM_8822B,
		       TX_DESC_NUM_8822B | ((RTL8822BE_SEG_NUM << 12) & 0x3000));
...
#if 0
static void _rtl8822be_hw_configure(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_pci *rtlpci = rtl_pcidev(rtl_pcipriv(hw));
	u32 reg_rrsr;

	reg_rrsr = RRSR_ALL_CCK | RRSR_ALL_OFDM_AG;
	/* Init value for RRSR. */
	rtl_write_dword(rtlpriv, REG_RRSR_8822B, reg_rrsr);

	/* ARFB table 9 for 11ac 5G 2SS */
	rtl_write_dword(rtlpriv, REG_ARFR0_8822B, 0x00000010);
	rtl_write_dword(rtlpriv, REG_ARFR0_8822B + 4, 0x3e0ff000);

	/* ARFB table 10 for 11ac 5G 1SS */
	rtl_write_dword(rtlpriv, REG_ARFR1_V1_8822B, 0x00000010);
	rtl_write_dword(rtlpriv, REG_ARFR1_V1_8822B + 4, 0x000ff000);

	/* Set SLOT time */
	rtl_write_byte(rtlpriv, REG_SLOT_8822B, 0x09);

	/* CF-End setting. */
	rtl_write_word(rtlpriv, REG_FWHW_TXQ_CTRL_8822B, 0x1F80);

	/* Set retry limit */
	rtl_write_word(rtlpriv, REG_RETRY_LIMIT_8822B, 0x0707);

	/* BAR settings */
	rtl_write_dword(rtlpriv, REG_BAR_MODE_CTRL_8822B, 0x0201ffff);

	/* Set Data / Response auto rate fallack retry count */
	rtl_write_dword(rtlpriv, REG_DARFRC_8822B, 0x01000000);
	rtl_write_dword(rtlpriv, REG_DARFRC_8822B + 4, 0x07060504);
	rtl_write_dword(rtlpriv, REG_RARFRC_8822B, 0x01000000);
	rtl_write_dword(rtlpriv, REG_RARFRC_8822B + 4, 0x07060504);

	/* Beacon related, for rate adaptive */
	rtl_write_byte(rtlpriv, REG_ATIMWND_8822B, 0x2);
	rtl_write_byte(rtlpriv, REG_BCN_MAX_ERR_8822B, 0xff);

	rtlpci->reg_bcn_ctrl_val = 0x1d;
	rtl_write_byte(rtlpriv, REG_BCN_CTRL_8822B, rtlpci->reg_bcn_ctrl_val);
	 ...
	 */
	rtl_write_byte(rtlpriv, REG_BCN_CTRL_CLINT0_8822B, 0);

	/* TBTT prohibit hold time. Suggested by designer TimChen. */
	rtl_write_byte(rtlpriv, REG_TBTT_PROHIBIT_8822B + 1 , 0xff); /* 8 ms */

	rtl_write_byte(rtlpriv, REG_PIFS_8822B, 0);
	rtl_write_byte(rtlpriv, REG_AGGR_BREAK_TIME_8822B, 0x16);

	rtl_write_word(rtlpriv, REG_NAV_PROT_LEN_8822B, 0x0040);
	rtl_write_word(rtlpriv, REG_PROT_MODE_CTRL_8822B, 0x08ff);

	/* For Rx TP. Suggested by SD1 Richard. Added by tynli. 2010.04.12.*/
	/*rtl_write_dword(rtlpriv, REG_FAST_EDCA_CTRL_8822B, 0x03086666);*/

	/* ACKTO for IOT issue. */
	rtl_write_byte(rtlpriv, REG_ACKTO_8822B, 0x40);

	/* Set Spec SIFS (used in NAV) */
	rtl_write_word(rtlpriv, REG_SPEC_SIFS_8822B, 0x100a);
	rtl_write_word(rtlpriv, REG_MAC_SPEC_SIFS_8822B, 0x100a);

	/* Set SIFS for CCK */
	rtl_write_word(rtlpriv, REG_SIFS_8822B, 0x100a);

	/* Set SIFS for OFDM */
	rtl_write_word(rtlpriv, REG_SIFS_TRX_8822B, 0x100a);

	/* Note Data sheet don't define */
	rtl_write_word(rtlpriv, 0x4C7, 0x80);

	rtl_write_byte(rtlpriv, REG_RX_PKT_LIMIT_8822B, 0x20);

	rtl_write_word(rtlpriv, REG_MAX_AGGR_NUM_8822B, 0x1717);

	/* Set Multicast Address. 2009.01.07. by tynli. */
	rtl_write_dword(rtlpriv, REG_MAR_8822B, 0xffffffff);
	rtl_write_dword(rtlpriv, REG_MAR_8822B + 4, 0xffffffff);
}
#endif
...
static bool _rtl8822be_check_pcie_dma_hang(struct rtl_priv *rtlpriv)
{
	...
	if (!(tmp & BIT(2))) {
		rtl_write_byte(rtlpriv, REG_DBI_FLAG_V1_8822B + 3, (tmp | BIT(2)));
					 ...
					 bool mac_power_on)
{
	...
	if (tmp & BIT(2)) {
		...
		rtl_write_byte(rtlpriv, REG_RXDMA_CONTROL_8822B, (tmp | BIT(2)));
...
int rtl8822be_hw_init(struct ieee80211_hw *hw)
{
...
#if 0
	tmp_u1b = rtl_read_byte(rtlpriv, REG_SYS_CLKR_8822B+1);
	u1byte = rtl_read_byte(rtlpriv, REG_CR_8822B);
	...
	if (err) {
		RT_TRACE(rtlpriv, COMP_ERR, DBG_EMERG, "halmac_init_hal failed\n");
	...
	rtl_write_byte(rtlpriv, REG_AFE_CTRL4_8822B, 0x2A);
	rtl_write_byte(rtlpriv, REG_AFE_CTRL4_8822B + 1, 0x00);
	rtl_write_byte(rtlpriv, REG_AFE_CTRL2_8822B, 0x83);
...
#if 1	/*defined(USING_RX_TAG)*/
	rtl_write_word(rtlpriv, REG_PCIE_CTRL_8822B, 0x8000);
	rtlhal->rx_tag = 1;
#else
	rtl_write_word(rtlpriv, REG_PCIE_CTRL_8822B, 0x0000);
	rtlhal->rx_tag = 0;
#endif

	rtl_write_byte(rtlpriv, REG_RX_DRVINFO_SZ_8822B, 0x4);

#if 0	/* included in halmac_init_hal() */
	err = rtl8822be_download_fw(hw , false);
	if (err) {
		RT_TRACE(rtlpriv, COMP_ERR, DBG_WARNING,
...
#if 0	/* included in halmac_init_hal() -- using callback function */
	rtl8822be_phy_mac_config(hw);

	rtl8822be_phy_bb_config(hw);

	rtl8822be_phy_rf_config(hw);
#endif
...
#if 0	/* not set_hal_start() yet. */
	/* set default channel */
	rtlphy->current_channel = 149;
	rtlphy->current_chan_bw = HT_CHANNEL_WIDTH_20;
	mac->cur_40_prime_sc = PRIME_CHNL_OFFSET_DONT_CARE;
	mac->bw_40 = false;
	mac->bw_80 = false;
	...
	if (ppsc->rfpwr_state == ERFON) {
		if (rtlphy->iqk_initialized) {
			rtl8822be_phy_iq_calibrate(hw, true);
		} else {
	...
	rtl_write_byte(rtlpriv, REG_NAV_UPPER_8822B, ((30000 + 127) / 128));
#endif

#if 0
	/*Fixed LDPC rx hang issue. */
	tmp_u4b = rtl_read_dword(rtlpriv, REG_SYS_SWR_CTRL1_8822B);
	rtl_write_byte(rtlpriv, REG_SYS_SWR_CTRL2_8822B, 0x75);
	tmp_u4b =  (tmp_u4b & 0xfff00fff) | (0x7E << 12);
	rtl_write_dword(rtlpriv, REG_SYS_SWR_CTRL1_8822B, tmp_u4b);
#endif

#if 0
	rtl8822be_dm_init(hw);
#endif
	...
	rtl_write_dword(rtlpriv, REG_HISR0_8822B, rtl_read_dword(rtlpriv, REG_HISR0_8822B));

	RT_TRACE(rtlpriv, COMP_INIT, DBG_LOUD,
		 "end of Rtl8822BE hw init %x\n", err);
	return 0;
}
...
static u32 _rtl8822be_read_chip_version(struct ieee80211_hw *hw)
{
	...
	if (value32 & TRP_VAUX_EN)
		version = (enum version_8822b) VERSION_TEST_CHIP_2T2R_8822B;
	else
		version = (enum version_8822b) VERSION_NORMAL_CHIP_2T2R_8822B;
#endif
...
int rtl8822be_set_network_type(struct ieee80211_hw *hw, enum nl80211_iftype type)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
...
void rtl8822be_set_qos(struct ieee80211_hw *hw, int aci)
{
	...
	rtl8822be_dm_init_edca_turbo(hw);
#endif
...
void rtl8822be_enable_interrupt(struct ieee80211_hw *hw)
{
	...
	rtl_write_dword(rtlpriv, REG_HIMR0_8822B, rtlpci->irq_mask[0] & 0xFFFFFFFF);
	rtl_write_dword(rtlpriv, REG_HIMR1_8822B, rtlpci->irq_mask[1] & 0xFFFFFFFF);
	rtl_write_dword(rtlpriv, REG_HIMR3_8822B, rtlpci->irq_mask[3] & 0xFFFFFFFF);
...
#if 0
static void _rtl8822be_poweroff_adapter(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
	u8 u1b_tmp;
...
#if 0	/* pwr seq is included in halmac */
	/* Run LPS WL RFOFF flow */
	rtl_hal_pwrseqcmdparsing(rtlpriv, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
				 PWR_INTF_PCI_MSK, Rtl8822B_NIC_LPS_ENTER_FLOW);
#endif
	/* turn off RF */
	rtl_write_byte(rtlpriv, REG_RF_CTRL_8822B, 0x00);

	/* ==== Reset digital sequence   ======  */
	if ((rtl_read_byte(rtlpriv, REG_MCUFWDL_8822B) & BIT(7)) && rtlhal->fw_ready)
		rtl8822be_firmware_selfreset(hw);

	/* Reset MCU  */
	u1b_tmp = rtl_read_byte(rtlpriv, REG_SYS_FUNC_EN_8822B + 1);
	rtl_write_byte(rtlpriv, REG_SYS_FUNC_EN_8822B + 1, (u1b_tmp & (~BIT(2))));

	/* reset MCU ready status */
	rtl_write_byte(rtlpriv, REG_MCUFWDL_8822B, 0x00);

#if 0	/* pwr seq is included in halmac */
	/* HW card disable configuration. */
	rtl_hal_pwrseqcmdparsing(rtlpriv, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
				 PWR_INTF_PCI_MSK, Rtl8822B_NIC_DISABLE_FLOW);
#endif

	/* Reset MCU IO Wrapper */
	u1b_tmp = rtl_read_byte(rtlpriv, REG_RSV_CTRL_8822B + 1);
	rtl_write_byte(rtlpriv, REG_RSV_CTRL_8822B + 1, (u1b_tmp & (~BIT(0))));
	u1b_tmp = rtl_read_byte(rtlpriv, REG_RSV_CTRL_8822B + 1);
	rtl_write_byte(rtlpriv, REG_RSV_CTRL_8822B + 1, (u1b_tmp | BIT(0)));

	/* lock ISO/CLK/Power control register */
	rtl_write_byte(rtlpriv, REG_RSV_CTRL_8822B, 0x0E);
}
#endif
...
#if 0
	_rtl8822be_poweroff_adapter(hw);
#else
...
void rtl8822be_interrupt_recognized(struct ieee80211_hw *hw,
				    ...
				    u32 *p_intc, u32 *p_intd)
{
	...
	*p_inta = rtl_read_dword(rtlpriv, REG_HISR0_8822B) & rtlpci->irq_mask[0];
	rtl_write_dword(rtlpriv, REG_HISR0_8822B, *p_inta);

	*p_intb = rtl_read_dword(rtlpriv, REG_HISR1_8822B) & rtlpci->irq_mask[1];
	rtl_write_dword(rtlpriv, REG_HISR1_8822B, *p_intb);

	*p_intd = rtl_read_dword(rtlpriv, REG_HISR3_8822B) & rtlpci->irq_mask[3];
...
void rtl8822be_update_interrupt_mask(struct ieee80211_hw *hw,
						 ...
						 bool autoload_fail, u8 *hwinfo)
{
	...
	_rtl8822be_read_power_value_fromprom(hw, &pwr2g, &pwr5g,
	...
	if (!autoload_fail) {
		efu->eeprom_regulatory = hwinfo[EEPROM_RF_BOARD_OPTION_8822B]
					 & 0x07;
				    ...
				    bool autoload_fail)
{
	...
	if (!autoload_fail) {
		...
		rtlhal->lna_type_2g = hwinfo[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8822B];
		...
		rtlhal->lna_type_5g = hwinfo[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8822B];
				    ...
				    bool autoload_fail)
{
	...
	u8 ext_type_pa_2g_a  = (hwinfo[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8822B] & BIT(2))      >> 2; /* 0xBD[2] */
	u8 ext_type_pa_2g_b  = (hwinfo[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8822B] & BIT(6))      >> 6; /* 0xBD[6] */
	u8 ext_type_pa_5g_a  = (hwinfo[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8822B] & BIT(2))      >> 2; /* 0xBF[2] */
	u8 ext_type_pa_5g_b  = (hwinfo[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8822B] & BIT(6))      >> 6; /* 0xBF[6] */
	u8 ext_type_lna_2g_a = (hwinfo[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8822B] & (BIT(1)|BIT(0))) >> 0; /* 0xBD[1:0] */
	u8 ext_type_lna_2g_b = (hwinfo[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8822B] & (BIT(5)|BIT(4))) >> 4; /* 0xBD[5:4] */
	u8 ext_type_lna_5g_a = (hwinfo[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8822B] & (BIT(1)|BIT(0))) >> 0; /* 0xBF[1:0] */
	u8 ext_type_lna_5g_b = (hwinfo[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8822B] & (BIT(5)|BIT(4))) >> 4; /* 0xBF[5:4] */
...
static void _rtl8822be_read_adapter_info(struct ieee80211_hw *hw)
{
	...
	/*customer ID*/
	rtlefuse->eeprom_oemid = *(u8 *) &hwinfo[EEPROM_CustomID_8822B];
	...
	/* channel plan */
	rtlefuse->eeprom_channelplan = *(u8 *) &hwinfo[EEPROM_ChannelPlan_8822B];
	...
	rtl8822be_read_bt_coexist_info_from_hwpg(hw, rtlefuse->autoload_failflag,
					       hwinfo);
	...
	/*board type*/
	rtlefuse->board_type = (((*(u8 *) &hwinfo[EEPROM_RF_BOARD_OPTION_8822B])
				& 0xE0) >> 5);
...
#if 0	/*reference phydm_init_hw_info_by_rfe_type_8822b()*/
	switch (rtlhal->rfe_type) {
	case 1:
		rtlhal->board_type = (ODM_BOARD_EXT_LNA|ODM_BOARD_EXT_LNA_5G|ODM_BOARD_EXT_PA|ODM_BOARD_EXT_PA_5G);
		rtlhal->package_type = 1;
		break;
	case 2:
		rtlhal->board_type = (ODM_BOARD_EXT_LNA_5G|ODM_BOARD_EXT_PA_5G);
		rtlhal->package_type = 2;
		break;
	case 3:
		rtlhal->board_type = 0;
		rtlhal->package_type = 2;
...
void rtl8822be_read_eeprom_info(struct ieee80211_hw *hw, struct rtl_phydm_params *params)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_efuse *rtlefuse = rtl_efuse(rtl_priv(hw));
	struct rtl_phy *rtlphy = &rtlpriv->phy;
...
static u8 _rtl8822be_get_ra_ldpc(struct ieee80211_hw *hw,
...
static bool _rtl8822be_get_ra_shortgi(struct ieee80211_hw *hw, struct ieee80211_sta *sta,
			      u8 mac_id)
{
					 ...
					 u8 rssi_level)
{
	...
	ratr_index = rtlpriv->phydm.ops->phydm_rate_id_mapping(rtlpriv,
							       wirelessmode,
							       rf_type,
							       rtlphy->current_chan_bw);
	sta_entry->ratr_index = ratr_index;

	rtlpriv->phydm.ops->phydm_get_ra_bitmap(rtlpriv,
						wirelessmode,
						rf_type,
						rtlphy->current_chan_bw,
						rssi_level,
...
void rtl8822be_set_key(struct ieee80211_hw *hw, u32 key_index,
