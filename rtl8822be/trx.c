 ...
 * DESC_RATEMCS0-->DESC_RATEMCS15 ==> idx is 0-->15
 */
static int _rtl8822be_rate_mapping(struct ieee80211_hw *hw,
...
static void _rtl8822be_query_rxphystatus(struct ieee80211_hw *hw,
					 u8 *phystrpt,
					 ...
					 struct rtl_stats *pstatus)
{
	...
	rtlpriv->phydm.ops->phydm_query_phy_status(rtlpriv, phystrpt,
						   hdr, pstatus);
			   ...
			   u8 *pdesc, struct sk_buff *skb)
{
	...
	rx_status->rate_idx = _rtl8822be_rate_mapping(hw,
						    status->is_ht,
						    status->is_vht,
						    status->rate);
	...
	_rtl8822be_translate_rx_signal_stuff(hw, skb, status,
						   p_phystrpt);
...
u16 rtl8822be_get_available_desc(struct ieee80211_hw *hw , u8 q_idx)
{
	...
	if(current_tx_read_point == current_tx_write_point)
		...
		point_diff = TX_DESC_NUM_8822B - (current_tx_write_point -
...
#if (DMA_IS_64BIT == 1)
	SET_TXBUFFER_DESC_ADD_HIGH_WITH_OFFSET(tx_bd_desc, 1, ((u64)data_addr >> 32));
#endif

	SET_TX_DESC_TXPKTSIZE(desc, (u16)(pkt_len));
}

static u8 rtl8822be_bw_mapping(struct ieee80211_hw *hw, struct rtl_tcb_desc *ptcb_desc)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
...
static u8 rtl8822be_sc_mapping(struct ieee80211_hw *hw, struct rtl_tcb_desc *ptcb_desc)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_phy *rtlphy = &rtlpriv->phy;
				...
				struct sk_buff *skb, u8 hw_queue)
{
	...
	rtl8822be_pre_fill_tx_bd_desc(hw, pbd_desc, pdesc, hw_queue, skb, mapping);
		      ...
		      u8 desc_name, u8 *val)
{
	...
	if (istx) {
		...
		case HW_DESC_OWN:{
			...
			tmp = rtl_read_dword(rtlpriv, get_desc_address_from_queue_index(q_idx));
		...
		case HW_DESC_RX_PREPARE:
			...
			SET_RX_BUFFER_PHYSICAL_LOW(pdesc, (*(dma_addr_t *)val) &
						DMA_BIT_MASK(32));
#if (DMA_IS_64BIT == 1)
			SET_RX_BUFFER_PHYSICAL_HIGH(pdesc, ((u64)(*(dma_addr_t *)val) >> 32));
#endif
			break;
		default:
			RT_ASSERT(false,
...
bool rtl8822be_is_tx_desc_closed(struct ieee80211_hw *hw, u8 hw_queue, u16 index)
{
	struct rtl_pci *rtlpci = rtl_pcidev(rtl_pcipriv(hw));
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	...
	tmpu32 = rtl_read_dword(rtlpriv, get_desc_address_from_queue_index(hw_queue));
	...
	ring->cur_tx_wp = cur_tx_wp;
	ring->cur_tx_rp = cur_tx_rp;
	ring->avl_desc = ((cur_tx_rp > cur_tx_wp) ? (cur_tx_rp - cur_tx_wp) : (TX_DESC_NUM_8822B - cur_tx_wp + cur_tx_rp));
...
void rtl8822be_tx_polling(struct ieee80211_hw *hw, u8 hw_queue)
{
	...
	if (hw_queue == BEACON_QUEUE) {
		/* kick start */
		rtl_write_byte(rtlpriv, REG_RX_RXBD_NUM_8822B + 1,
			   rtl_read_byte(rtlpriv, REG_RX_RXBD_NUM_8822B + 1) | BIT(4));
	}
}
