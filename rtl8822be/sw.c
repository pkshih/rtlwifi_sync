...
int rtl8822be_init_sw_vars(struct ieee80211_hw *hw)
{
...
#if 0
	/*just 5G band*/
	rtlpriv->rtlhal.current_bandtype = BAND_ON_5G;
	...
	rtlpci->irq_mask[1] = (u32) (IMR_RXFOVW |
						IMR_TXFOVW |
						0);
...
static struct rtl_hal_ops rtl8822be_hal_ops = {
	...
	.halmac_cb_init_bb_rf_register = rtl8822be_halmac_cb_init_bb_rf_register,
	.halmac_cb_write_data_rsvd_page = rtl8822b_halmac_cb_write_data_rsvd_page,
...
module_param_named(disable_watchdog, rtl8822be_mod_params.disable_watchdog, bool, 0444);
...
#else
static int __init rtl8822be_module_init(void)
{
	int ret;

	ret = pci_register_driver(&rtl8822be_driver);
	if (ret)
...
static void __exit rtl8822be_module_exit(void)
{
	pci_unregister_driver(&rtl8822be_driver);
}

module_init(rtl8822be_module_init);
module_exit(rtl8822be_module_exit);
#endif
