				      ...
				      struct dm_iqk_info *iqk_info, u8 status)
{
	...
	while (1) {
		...
		if (status == 0)
			ODM_RT_TRACE(dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE,
...
void _iqk_iqk_fail_report_8822b(struct phy_dm_struct *dm)
{
	u32 tmp1bf0 = 0x0;
	u8 i;
	...
	for (i = 0; i < 4; i++) {
		if (tmp1bf0 & (0x1 << i))
			ODM_RT_TRACE(dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
				     ("[IQK] please check S%d TXIQK\n", i));
		if (tmp1bf0 & (0x1 << (i + 12)))
			ODM_RT_TRACE(dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
				     ("[IQK] please check S%d RXIQK\n", i));
	}
}

void _iqk_backup_mac_bb_8822b(struct phy_dm_struct *dm, u32 *MAC_backup,
...
void _iqk_backup_rf_8822b(struct phy_dm_struct *dm, u32 RF_backup[][2],
			  u32 *backup_rf_reg)
{
	u32 i;
...
void _iqk_agc_bnd_int_8822b(struct phy_dm_struct *dm)
{
...
void _iqk_bb_reset_8822b(struct phy_dm_struct *dm)
{
	bool cca_ing = false;
	u32 count = 0;
...
void _iqk_afe_setting_8822b(struct phy_dm_struct *dm, bool do_iqk)
{
	if (do_iqk) {
...
void _iqk_restore_mac_bb_8822b(struct phy_dm_struct *dm, u32 *MAC_backup,
			       u32 *BB_backup, u32 *backup_mac_reg,
			       u32 *backup_bb_reg)
{
	u32 i;
...
void _iqk_restore_rf_8822b(struct phy_dm_struct *dm, u32 *backup_rf_reg,
			   u32 RF_backup[][2])
{
	u32 i;
...
void _iqk_backup_iqk_8822b(struct phy_dm_struct *dm, u8 step)
{
	struct dm_iqk_info *iqk_info = &dm->IQK_info;
	u8 path;
	...
	if (step == 0x0) {
		...
		for (path = 0; path < 2; path++) {
			iqk_info->lok_idac[0][path] = odm_get_rf_reg(
				dm, (enum odm_rf_radio_path_e)path, 0x58,
				RFREGOFFSETMASK);
...
void _iqk_reload_iqk_setting_8822b(
	...
	)
{
	...
	for (path = 0; path < 2; path++) {
		if (reload_idx == 2) {
			odm_set_rf_reg(dm, (enum odm_rf_radio_path_e)path, 0xdf,
				       BIT(4), 0x1);
			odm_set_rf_reg(dm, (enum odm_rf_radio_path_e)path, 0x58,
				       RFREGOFFSETMASK,
...
bool _iqk_reload_iqk_8822b(struct phy_dm_struct *dm, bool reset)
{
	struct dm_iqk_info *iqk_info = &dm->IQK_info;
	u8 i;
	bool reload = false;
	...
	if (reset) {
		...
		for (i = 0; i < 2; i++) {
			if (iqk_info->rf_reg18 == iqk_info->iqk_channel[i]) {
				...
				ODM_RT_TRACE(
					dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
...
void _iqk_rfe_setting_8822b(struct phy_dm_struct *dm, bool ext_pa_on)
{
	if (ext_pa_on) {
		/*RFE setting*/
...
void _iqk_rf_setting_8822b(struct phy_dm_struct *dm)
{
	u8 path;
	u32 tmp;
	...
	for (path = 0; path < 2; path++) {
		...
		tmp = odm_get_rf_reg(dm, (enum odm_rf_radio_path_e)path, 0xdf,
				     RFREGOFFSETMASK);
		...
		/*release 0x56 TXBB*/
		odm_set_rf_reg(dm, (enum odm_rf_radio_path_e)path, 0x65,
			       RFREGOFFSETMASK, 0x09000);

		if (*dm->p_band_type == ODM_BAND_5G) {
			odm_set_rf_reg(dm, (enum odm_rf_radio_path_e)path, 0xef,
				       BIT(19), 0x1);
			odm_set_rf_reg(dm, (enum odm_rf_radio_path_e)path, 0x33,
				       RFREGOFFSETMASK, 0x00026);
			odm_set_rf_reg(dm, (enum odm_rf_radio_path_e)path, 0x3e,
				       RFREGOFFSETMASK, 0x00037);
			odm_set_rf_reg(dm, (enum odm_rf_radio_path_e)path, 0x3f,
...
void _iqk_configure_macbb_8822b(struct phy_dm_struct *dm)
{
...
void _iqk_lok_setting_8822b(struct phy_dm_struct *dm, u8 path)
{
	odm_write_4byte(dm, 0x1b00, 0xf8000008 | path << 1);
	odm_write_4byte(dm, 0x1bcc, 0x9);
	odm_write_1byte(dm, 0x1b23, 0x00);

	switch (*dm->p_band_type) {
	case ODM_BAND_2_4G:
		odm_write_1byte(dm, 0x1b2b, 0x00);
		odm_set_rf_reg(dm, (enum odm_rf_radio_path_e)path, 0x56,
			       RFREGOFFSETMASK, 0x50df2);
		odm_set_rf_reg(dm, (enum odm_rf_radio_path_e)path, 0x8f,
			       RFREGOFFSETMASK, 0xadc00);
		/* WE_LUT_TX_LOK*/
		odm_set_rf_reg(dm, (enum odm_rf_radio_path_e)path, 0xef, BIT(4),
			       0x1);
		odm_set_rf_reg(dm, (enum odm_rf_radio_path_e)path, 0x33,
			       BIT(1) | BIT(0), 0x0);
		break;
...
void _iqk_txk_setting_8822b(struct phy_dm_struct *dm, u8 path)
{
	odm_write_4byte(dm, 0x1b00, 0xf8000008 | path << 1);
	odm_write_4byte(dm, 0x1bcc, 0x9);
...
void _iqk_rxk1_setting_8822b(struct phy_dm_struct *dm, u8 path)
{
	odm_write_4byte(dm, 0x1b00, 0xf8000008 | path << 1);
...
void _iqk_rxk2_setting_8822b(struct phy_dm_struct *dm, u8 path, bool is_gs)
{
	struct dm_iqk_info *iqk_info = &dm->IQK_info;
...
bool _iqk_check_cal_8822b(struct phy_dm_struct *dm, u32 IQK_CMD)
{
...
bool _iqk_rx_iqk_gain_search_fail_8822b(struct phy_dm_struct *dm, u8 path,
					u8 step)
{
	struct dm_iqk_info *iqk_info = &dm->IQK_info;
	bool fail = true;
	...
	if (step == RXIQK2) {
		rf_reg0 = odm_get_rf_reg(dm, (enum odm_rf_radio_path_e)path,
					 0x0, RFREGOFFSETMASK);
		odm_write_4byte(dm, 0x1b00, 0xf8000008 | path << 1);
		ODM_RT_TRACE(
			dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE,
...
bool _lok_one_shot_8822b(void *dm_void, u8 path)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	struct dm_iqk_info *iqk_info = &dm->IQK_info;
	u8 delay_count = 0;
...
bool _iqk_one_shot_8822b(void *dm_void, u8 path, u8 idx)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	struct dm_iqk_info *iqk_info = &dm->IQK_info;
	u8 delay_count = 0;
	...
	if (idx == TXIQK) {
		IQK_CMD = 0xf8000008 | ((*dm->p_band_width + 4) << 8) |
			  (1 << (path + 4));
		ODM_RT_TRACE(dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE,
		...
		IQK_CMD = 0xf8000008 | ((*dm->p_band_width + 9) << 8) |
			  (1 << (path + 4));
		ODM_RT_TRACE(dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE,
	...
	if (idx == RXIQK2) {
		iqk_info->rxiqk_agc[0][path] = (u16)(
			((odm_get_rf_reg(dm, (enum odm_rf_radio_path_e)path,
					 0x0, RFREGOFFSETMASK) >>
			  5) &
			 0xff) |
			(iqk_info->tmp1bcc << 8));
	...
	case 1: /*gain search_RXK1*/
		...
		while (1) {
			KFAIL = _iqk_rx_iqk_gain_search_fail_8822b(dm, path,
								   RXIQK1);
			if (KFAIL &&
			    (iqk_info->gs_retry_count[0][path][RXIQK1] < 2))
				iqk_info->gs_retry_count[0][path][RXIQK1]++;
			else if (KFAIL) {
	...
	case 2: /*gain search_RXK2*/
		...
		while (1) {
			KFAIL = _iqk_rx_iqk_gain_search_fail_8822b(dm, path,
								   RXIQK2);
			if (KFAIL &&
			    (iqk_info->gs_retry_count[0][path][RXIQK2] <
			     rxiqk_gs_limit))
				iqk_info->gs_retry_count[0][path][RXIQK2]++;
			else {
				iqk_info->rxiqk_step++;
...
void _iqk_iqk_by_path_8822b(void *dm_void, bool segment_iqk)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	struct dm_iqk_info *iqk_info = &dm->IQK_info;
	bool KFAIL = true;
...
void _iqk_start_iqk_8822b(struct phy_dm_struct *dm, bool segment_iqk)
{
	u32 tmp;
...
void _iq_calibrate_8822b_init(void *dm_void)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	struct dm_iqk_info *iqk_info = &dm->IQK_info;
	u8 i, j;
...
void _phy_iq_calibrate_8822b(struct phy_dm_struct *dm, bool reset)
{
	...
	ODM_RT_TRACE(
		...
		("[IQK]p_band_type = %s, band_width = %d, ExtPA2G = %d, ext_pa_5g = %d\n",
		 (*dm->p_band_type == ODM_BAND_5G) ? "5G" : "2G",
		 *dm->p_band_width, dm->ext_pa, dm->ext_pa_5g));
	ODM_RT_TRACE(dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
...
void _phy_iq_calibrate_by_fw_8822b(void *dm_void, u8 clear) {}
