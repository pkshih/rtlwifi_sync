...
bool phydm_rfe_8822b(struct phy_dm_struct *dm, u8 channel)
{
	if (dm->rfe_type == 4) {
		...
		if (((dm->cut_version == ODM_CUT_A) ||
			...
			ODM_RT_TRACE(
				dm, ODM_PHY_CONFIG, ODM_DBG_TRACE,
...
void phydm_ccapar_by_rfe_8822b(struct phy_dm_struct *dm)
{
	...
	ODM_RT_TRACE(dm, ODM_PHY_CONFIG, ODM_DBG_TRACE,
		     ("%s: (Pkt%d, Intf%d, RFE%d), row = %d, col = %d\n",
		      __func__, dm->package_type, dm->support_interface,
		      dm->rfe_type, row, col));
}

void phydm_ccapar_by_bw_8822b(struct phy_dm_struct *dm, enum odm_bw_e bandwidth)
{
	u32 reg82c;
...
void phydm_ccapar_by_rxpath_8822b(struct phy_dm_struct *dm)
{
	if (dm->cut_version != ODM_CUT_A)
		return;
...
void phydm_rxdfirpar_by_bw_8822b(struct phy_dm_struct *dm,
				 ...
				 enum odm_rf_path_e rx_path, bool is_tx2_path)
{
	...
	if ((dm->mp_mode) || *dm->p_antenna_test || (dm->normal_rx_path)) {
