...
bool get_mix_mode_tx_agc_bb_swing_offset_8822b(void *dm_void,
					       ...
					       u8 tx_power_index_offest)
{
	...
	ODM_RT_TRACE(
		dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
				   ...
				   u8 rf_path, u8 channel_mapped_index)
{
	...
	if (dm->mp_mode) {
		...
		u16 rate = *dm->p_forced_data_rate;
	...
	if (dm->number_linked_client != 0)
		tx_power_index = odm_get_tx_power_index(
			dm, (enum odm_rf_radio_path_e)rf_path, tx_rate,
			band_width, channel);
...
void _phy_lc_calibrate_8822b(struct phy_dm_struct *dm)
{
	u32 lc_cal = 0, cnt = 0;
...
void configure_txpower_track_8822b(struct txpwrtrack_cfg *p_config)
{
...
bool _phy_query_rf_path_switch_8822b(struct phy_dm_struct *dm)
{
...
bool phy_query_rf_path_switch_8822b(struct phy_dm_struct *dm)
{
