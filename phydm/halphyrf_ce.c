...
void configure_txpower_track(void *dm_void, struct txpwrtrack_cfg *p_config)
{
...
void odm_txpowertracking_callback_thermal_meter(void *dm_void)
{
	...
	u8 OFDM_min_index = 0;
	/* get_right_chnl_place_for_iqk(p_hal_data->current_channel) */
	...
	/*cali_info->txpowertrack_control = p_hal_data->txpowertrack_control;
...
void odm_iq_calibrate(struct phy_dm_struct *dm)
{
	void *adapter = dm->adapter;
	...
	if (dm->is_linked) {
		if ((*dm->p_channel != dm->pre_channel) &&
		    (!*dm->p_is_scan_in_process)) {
			dm->pre_channel = *dm->p_channel;
