...
void odm_set_rf_reg(struct phy_dm_struct *dm,
		    enum odm_rf_radio_path_e e_rf_path, u32 reg_addr,
...
u32 odm_get_rf_reg(struct phy_dm_struct *dm, enum odm_rf_radio_path_e e_rf_path,
 ...
 */
void odm_allocate_memory(struct phy_dm_struct *dm, void **p_ptr, u32 length)
{
	*p_ptr = kmalloc(length, GFP_ATOMIC);
}

/* length could be ignored, used to detect memory leakage. */
void odm_free_memory(struct phy_dm_struct *dm, void *p_ptr, u32 length)
{
	kfree(p_ptr);
}

void odm_move_memory(struct phy_dm_struct *dm, void *p_dest, void *p_src,
		     u32 length)
{
...
s32 odm_compare_memory(struct phy_dm_struct *dm, void *p_buf1, void *p_buf2,
		       u32 length)
{
...
void odm_set_timer(struct phy_dm_struct *dm, struct timer_list *p_timer,
		   u32 ms_delay)
{
	mod_timer(p_timer, jiffies + msecs_to_jiffies(ms_delay));
}

void odm_initialize_timer(struct phy_dm_struct *dm, struct timer_list *p_timer,
			  void *call_back_func, void *p_context,
			  const char *sz_id)
{
	init_timer(p_timer);
	p_timer->function = call_back_func;
	p_timer->data = (unsigned long)dm;
	/*mod_timer(p_timer, jiffies+RTL_MILISECONDS_TO_JIFFIES(10));	*/
}

void odm_cancel_timer(struct phy_dm_struct *dm, struct timer_list *p_timer)
{
	del_timer(p_timer);
}

void odm_release_timer(struct phy_dm_struct *dm, struct timer_list *p_timer) {}

u8 phydm_trans_h2c_id(struct phy_dm_struct *dm, u8 phydm_h2c_id)
{
			  ...
			  u8 band_width, u8 channel)
{
	...
	return phy_get_tx_power_index(adapter,
