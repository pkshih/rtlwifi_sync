...
bool phydm_api_set_txagc(struct phy_dm_struct *, u32, enum odm_rf_radio_path_e,
			 u8, bool);
					   ...
					   char *output)
{
	if ((u8)dm_value[2] != 0xff) {
		if (phydm_api_set_txagc(dm, dm_value[3],
					(enum odm_rf_radio_path_e)dm_value[1],
					(u8)dm_value[2], true))
...
void phydm_release_bb_dbg_port(void *dm_void)
{
	...
	ODM_RT_TRACE(dm, ODM_COMP_API, ODM_DBG_LOUD, ("Release BB dbg_port\n"));
}
...
void phydm_bb_rx_hang_info(void *dm_void, u32 *_used, char *output,
			   u32 *_out_len)
{
	u32 value32 = 0;
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
...
void phydm_bb_debug_info(void *dm_void, u32 *_used, char *output, u32 *_out_len)
{
	...
	if (rx_ht != 0) {
		if (rxsc == 0)
			...
			tmp_string = "lsc40";
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "  %-35s", tmp_string));
	}
	...
	if (dm->support_ic_type & ODM_RTL8822B) {
		...
		if (*dm->p_band_width == ODM_BW80M) {
			factor = "256/234";
		} else if (*dm->p_band_width == ODM_BW40M) {
			factor = "128/108";
		} else if (*dm->p_band_width == ODM_BW20M) {
			if (rx_ht != 2 || rx_ht != 1)
...
void phydm_basic_dbg_message(void *dm_void)
{
	...
	if (dm->is_linked) {
		ODM_RT_TRACE(dm, ODM_COMP_COMMON, ODM_DBG_LOUD,
			     ("ID=%d, BW=((%d)), CH=((%d))\n",
			      dm->curr_station_id, 20 << *dm->p_band_width,
			      *dm->p_channel));

		/*Print RX rate*/
		if (dm->rx_rate <= ODM_RATE11M)
			ODM_RT_TRACE(
				dm, ODM_COMP_COMMON, ODM_DBG_LOUD,
			   ...
			   u32 monitor_mode, u32 macid)
{
	...
	ODM_RT_TRACE(dm, ODM_FW_DEBUG_TRACE, ODM_DBG_LOUD, ("---->\n"));
	if (monitor_mode == 0)
		ODM_RT_TRACE(dm, ODM_FW_DEBUG_TRACE, ODM_DBG_LOUD,
			     ("[H2C] FW_debug_en: (( %d ))\n", enable));
	else
		ODM_RT_TRACE(
			dm, ODM_FW_DEBUG_TRACE, ODM_DBG_LOUD,
...
u8 phydm_api_get_txagc(struct phy_dm_struct *dm, enum odm_rf_radio_path_e path,
		       u8 hw_rate)
{
	u8 ret = 0;
...
bool phydm_api_switch_bw_channel(struct phy_dm_struct *dm, u8 central_ch,
				 u8 primary_ch_idx, enum odm_bw_e bandwidth)
{
	bool ret = false;
...
bool phydm_api_trx_mode(struct phy_dm_struct *dm, enum odm_rf_path_e tx_path,
	...
	if (((dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8197F)) &&
		...
		for (rate_idx = 0; rate_idx <= 0x53; rate_idx++) {
			...
			txagc = phydm_api_get_txagc(
				dm, (enum odm_rf_radio_path_e)path, rate_idx);
...
void phydm_debug_trace(void *dm_void, u32 *const dm_value, u32 *_used,
		       char *output, u32 *_out_len)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
		...
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "06. (( %s ))ANT_DIV\n",
			 ((dm->debug_components & ODM_COMP_ANT_DIV) ? ("V") :
								      ("."))));
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "07. (( %s ))DFS\n",
		...
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "30. (( %s ))COMMON\n",
			 ((dm->debug_components & ODM_COMP_COMMON) ? ("V") :
								     ("."))));
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "31. (( %s ))API\n",
			  ...
			  char *output, u32 *_out_len)
{
	...
	if (dm_value[0] == 100) {
		...
		PHYDM_SNPRINTF((output + used, out_len - used,
				"00. (( %s ))RA\n",
				((dm->fw_debug_components & PHYDM_FW_COMP_RA) ?
					 ("V") :
					 ("."))));
...
void phydm_dump_all_reg(void *dm_void, u32 *_used, char *output, u32 *_out_len)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	u32 addr = 0;
	u32 used = *_used;
	...
	/* dump BB register */
	PHYDM_VAST_INFO_SNPRINTF(
		(output + used, out_len - used, "BB==========\n"));
	phydm_dump_bb_reg(dm, &used, output, &out_len);

	/* dump RF register */
	PHYDM_VAST_INFO_SNPRINTF(
		(output + used, out_len - used, "RF-A==========\n"));
	for (addr = 0; addr < 0xFF; addr++)
		PHYDM_VAST_INFO_SNPRINTF(
			(output + used, out_len - used, "0x%02x 0x%05x\n", addr,
			 odm_get_rf_reg(dm, ODM_RF_PATH_A, addr,
					RFREGOFFSETMASK)));

	if (dm->rf_type > ODM_1T1R) {
		PHYDM_VAST_INFO_SNPRINTF(
			(output + used, out_len - used, "RF-B==========\n"));
		for (addr = 0; addr < 0xFF; addr++)
			PHYDM_VAST_INFO_SNPRINTF(
				(output + used, out_len - used,
				 "0x%02x 0x%05x\n", addr,
...
void phydm_enable_big_jump(struct phy_dm_struct *dm, bool state)
{
	struct dig_thres *dig_tab = &dm->dm_dig_table;
...
void phydm_show_rx_rate(struct phy_dm_struct *dm, u32 *_used, char *output,
			u32 *_out_len)
{
	u32 used = *_used;
	u32 out_len = *_out_len;
		      ...
		      u32 input_num, u8 flag, char *output, u32 out_len)
{
	...
	if (input_num) {
		phydm_ary_size =
			sizeof(phy_dm_ary) / sizeof(struct _PHYDM_COMMAND);
	...
	case PHYDM_DYNAMIC_RA_PATH:
		...
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "Not Support IC"));
