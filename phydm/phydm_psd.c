...
u32 phydm_get_psd_data(void *dm_void, u32 psd_tone_idx, u32 igi)
{
	...
	struct psd_info *p_dm_psd_table = &dm->dm_psd_table;
	u32 psd_report = 0;

	odm_set_bb_reg(dm, p_dm_psd_table->psd_reg, 0x3ff, psd_tone_idx);

	odm_set_bb_reg(dm, p_dm_psd_table->psd_reg, BIT(22),
		       1); /*PSD trigger start*/
	ODM_delay_us(10);
	odm_set_bb_reg(dm, p_dm_psd_table->psd_reg, BIT(22),
		       0); /*PSD trigger stop*/

	psd_report = odm_get_bb_reg(dm, p_dm_psd_table->psd_report_reg, 0xffff);
	psd_report = odm_convert_to_db(psd_report) + igi;

	return psd_report;
}

u8 phydm_psd_stop_trx(void *dm_void)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	u32 i;
...
void phydm_psd(void *dm_void, u32 igi, u16 start_point, u16 stop_point)
{
	...
	ODM_RT_TRACE(dm, ODM_COMP_API, ODM_DBG_LOUD, ("PSD Start =>\n"));
	...
	/*[back up IGI]*/
	p_dm_psd_table->initial_gain_backup =
	...
	for (i = start_point; i <= stop_point; i++) {
		...
		psd_result =
			(u8)((psd_result_tmp / p_dm_psd_table->sw_avg_time)) -
			p_dm_psd_table->psd_pwr_common_offset;

		if (p_dm_psd_table->fft_smp_point == 128 &&
		    (p_dm_psd_table->noise_k_en)) {
	...
	ODM_RT_TRACE(dm, ODM_COMP_API, ODM_DBG_LOUD, ("PSD finished\n\n"));
			    ...
			    u8 psd_input, u8 channel, u8 noise_k_en)
{
	...
	/*bw = (*dm->p_band_width); //ODM_BW20M */
	/*channel = *(dm->p_channel);*/
}
...
void phydm_psd_init(void *dm_void)
{
	...
	ODM_RT_TRACE(dm, ODM_COMP_API, ODM_DBG_LOUD, ("PSD para init\n"));
