...
static bool rtl_phydm_phy_rf_config(struct rtl_priv *rtlpriv)
{
	...
	struct rtl_phy *rtlphy = &rtlpriv->phy;
	enum hal_status status;
	enum odm_rf_radio_path_e rfpath;
			       ...
			       bool is_tx2_path)
{
	...
	if (IS_HARDWARE_TYPE_8822B(rtlpriv))
		return config_phydm_trx_mode_8822b(dm, tx_path, rx_path,
						   is_tx2_path);
				 ...
				 enum radio_path rfpath, u32 addr, u32 mask)
{
	...
	if (IS_HARDWARE_TYPE_8822B(rtlpriv))
		return config_phydm_read_rf_reg_8822b(dm, rfpath, addr, mask);
				   ...
				   u32 data)
{
	...
	if (IS_HARDWARE_TYPE_8822B(rtlpriv))
		return config_phydm_write_rf_reg_8822b(dm, rfpath, addr, mask,
						       data);
			       ...
			       u8 hw_rate)
{
	...
	if (IS_HARDWARE_TYPE_8822B(rtlpriv))
		return config_phydm_read_txagc_8822b(dm, rfpath, hw_rate);
				  ...
				  enum radio_path rfpath, u8 hw_rate)
{
	...
	if (IS_HARDWARE_TYPE_8822B(rtlpriv))
		return config_phydm_write_txagc_8822b(dm, power_index, rfpath,
						      hw_rate);
