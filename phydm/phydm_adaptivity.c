...
void phydm_check_adaptivity(void *dm_void)
{
	...
	struct _ADAPTIVITY_STATISTICS *adaptivity =
		(struct _ADAPTIVITY_STATISTICS *)phydm_get_structure(
			dm, PHYDM_ADAPTIVITY);
...
void phydm_set_lna(void *dm_void, enum phydm_set_lna type)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
...
void phydm_mac_edcca_state(void *dm_void, enum phydm_mac_edcca_type state)
{
	...
	ODM_RT_TRACE(dm, PHYDM_COMP_ADAPTIVITY, ODM_DBG_LOUD,
		     ("EDCCA enable state = %d\n", state));
}
...
bool phydm_re_search_condition(void *dm_void)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
...
void phydm_adaptivity_info_init(void *dm_void, enum phydm_adapinfo_e cmn_info,
				u32 value)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	struct _ADAPTIVITY_STATISTICS *adaptivity =
...
void phydm_adaptivity_init(void *dm_void)
{
	...
	ODM_RT_TRACE(
		dm, PHYDM_COMP_ADAPTIVITY, ODM_DBG_LOUD,
...
void phydm_adaptivity(void *dm_void)
{
	...
	if (*dm->p_band_width == ODM_BW20M) /*CHANNEL_WIDTH_20*/
		igi_target = adaptivity->igi_base;
	else if (*dm->p_band_width == ODM_BW40M)
		igi_target = adaptivity->igi_base + 2;
	else if (*dm->p_band_width == ODM_BW80M)
		igi_target = adaptivity->igi_base + 2;
	else
		igi_target = adaptivity->igi_base;
	...
	ODM_RT_TRACE(
		...
		("band_width=%s, igi_target=0x%x, dynamic_link_adaptivity = %d, acs_for_adaptivity = %d\n",
		 (*dm->p_band_width == ODM_BW80M) ?
			 "80M" :
			 ((*dm->p_band_width == ODM_BW40M) ? "40M" : "20M"),
