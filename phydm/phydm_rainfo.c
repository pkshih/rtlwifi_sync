			...
			char *output, u32 *_out_len)
{
	...
	if (dm_value[0] == 100) {
		...
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "[Set] Error\n"));
		/**/
	}
}
...
void odm_c2h_ra_para_report_handler(void *dm_void, u8 *cmd_buf, u8 cmd_len)
{
	...
	ODM_RT_TRACE(dm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
	...
	if (para_idx == RADBG_DEBUG_MONITOR1) {
		...
		ODM_RT_TRACE(dm, ODM_FW_DEBUG_TRACE, ODM_DBG_LOUD,
			     ("%5s  0x%x%x%x%x%x\n", "RA mask =", cmd_buf[11],
			      cmd_buf[10], cmd_buf[9], cmd_buf[8], cmd_buf[7]));
	}
}
...
void phydm_ra_dynamic_retry_count(void *dm_void)
{
	...
	if (dm->pre_b_noisy != dm->noisy_decision) {
		if (dm->noisy_decision) {
			ODM_RT_TRACE(dm, ODM_COMP_RATE_ADAPTIVE, ODM_DBG_LOUD,
		 ...
		 (vht_en) ? ((rate_idx - ODM_RATEVHTSS1MCS0) % 10) :
			    ((rate_idx >= ODM_RATEMCS0) ?
				     ...
				     ((rate_idx <= ODM_RATE54M) ?
					      ...
					      0)),
		 (b_sgi) ? "-S" : "  ", (rate_idx >= ODM_RATEMCS0) ? "" : "M"));
}
...
void odm_rssi_monitor_check_mp(void *dm_void) {}

void odm_rssi_monitor_check_ce(void *dm_void)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	struct ra_table *ra_tab = &dm->dm_ra_table;
	...
	for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
		entry = (struct rtl_sta_info *)dm->p_odm_sta_info[i];
...
void odm_rssi_monitor_check_ap(void *dm_void) {}
...
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	struct _ODM_RATE_ADAPTIVE *p_odm_ra = &dm->rate_adaptive;
...
void odm_refresh_rate_adaptive_mask(void *dm_void)
{
	...
	if (!(dm->support_ability & ODM_BB_RA_MASK)) {
		ODM_RT_TRACE(dm, ODM_COMP_RA_MASK, ODM_DBG_TRACE,
...
u8 phydm_trans_platform_bw(void *dm_void, u8 BW)
{
...
u8 phydm_trans_platform_rf_type(void *dm_void, u8 rf_type)
{
	if (rf_type == RF_1T2R)
...
u32 phydm_trans_platform_wireless_mode(void *dm_void, u32 wireless_mode)
{
...
void phydm_ra_common_info_update(void *dm_void)
{
	...
	if (dm->number_linked_client != 0) {
		...
		ODM_RT_TRACE(
			dm, ODM_COMP_RATE_ADAPTIVE, ODM_DBG_LOUD,
...
void odm_set_ra_dm_arfb_by_noisy(struct phy_dm_struct *dm) {}
...
void phydm_update_pwr_track(void *dm_void, u8 rate)
{
	...
	ODM_RT_TRACE(dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
...
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	struct _ODM_RATE_ADAPTIVE *p_ra = &dm->rate_adaptive;
	...
	default:
		ODM_RT_ASSERT(dm, false, ("wrong rssi level setting %d !",
					  *p_ra_tr_state));
		break;
	}
