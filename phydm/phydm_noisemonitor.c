...
s16 odm_inband_noise_monitor_n_series(struct phy_dm_struct *dm, u8 is_pause_dig,
				      u8 igi_value, u32 max_time)
{
	...
	reg_c50 &= ~BIT(7);
	ODM_RT_TRACE(dm, ODM_COMP_COMMON, ODM_DBG_LOUD,
		     ("0x%x = 0x%02x(%d)\n", REG_OFDM_0_XA_AGC_CORE1, reg_c50,
		      reg_c50));
...
s16 odm_inband_noise_monitor_ac_series(struct phy_dm_struct *dm,
