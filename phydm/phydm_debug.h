 ...
 *	For example, IRP/Packet/OID canceled, device suprisely unremoved and etc
 */
...
#define RT_PRINTK(fmt, args...)                                                \
	RT_TRACE(((struct rtl_priv *)dm->adapter), COMP_PHYDM, DBG_DMESG, fmt, \
		 ##args)
...
#define ODM_RT_TRACE(dm, comp, level, fmt)                                     \
	do {                                                                   \
		if (((comp) & dm->debug_components) &&                         \
		    (level <= dm->debug_level || level == ODM_DBG_SERIOUS)) {  \
...
#define ODM_RT_TRACE_F(dm, comp, level, fmt)                                   \
	do {                                                                   \
		if (((comp) & dm->debug_components) &&                         \
		    (level <= dm->debug_level)) {                              \
			RT_PRINTK fmt;                                         \
		}                                                              \
	} while (0)

#define ODM_RT_ASSERT(dm, expr, fmt)                                           \
	do {                                                                   \
		if (!(expr)) {                                                 \
...
#define ODM_PRINT_ADDR(dm, comp, level, title_str, ptr)                        \
	do {                                                                   \
		if (((comp) & dm->debug_components) &&                         \
		    (level <= dm->debug_level)) {                              \
			int __i;                                               \
