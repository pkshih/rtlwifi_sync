...
void odm_fa_threshold_check(void *dm_void, bool is_dfs_band,
...
u8 odm_forbidden_igi_check(void *dm_void, u8 dig_dynamic_min, u8 current_igi)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	struct dig_thres *dig_tab = &dm->dm_dig_table;
	...
	if (fa_cnt->cnt_all > 10000) {
		...
		ODM_RT_TRACE(dm, ODM_COMP_DIG, ODM_DBG_LOUD,
...
void odm_inband_noise_calculate(void *dm_void) { return; }

void odm_dig_for_bt_hs_mode(void *dm_void) {}

void phydm_set_big_jump_step(void *dm_void, u8 current_igi)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	struct dig_thres *dig_tab = &dm->dm_dig_table;
...
void odm_write_dig(void *dm_void, u8 current_igi)
{
	...
	ODM_RT_TRACE(dm, ODM_COMP_DIG, ODM_DBG_TRACE,
	...
	case PHYDM_RESUME: {
		...
		ODM_RT_TRACE(dm, ODM_COMP_DIG, ODM_DBG_LOUD,
			     ("%s(): Write IGI (0x%x) of level (%d)\n",
			      __func__, dig_tab->pause_dig_value[max_level],
			      max_level));
		break;
	}
	default:
...
bool odm_dig_abort(void *dm_void)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	struct dig_thres *dig_tab = &dm->dm_dig_table;
...
void odm_DIG(void *dm_void)
{
	...
	ODM_RT_TRACE(dm, ODM_COMP_DIG, ODM_DBG_LOUD, ("DIG Start===>\n"));
	...
	/* 1 Boundary Decision */
	{
		...
		if ((((*dm->p_channel >= 52) && (*dm->p_channel <= 64)) ||
		     ((*dm->p_channel >= 100) && (*dm->p_channel <= 140))) &&
		    phydm_dfs_master_enabled(dm)) {
			is_dfs_band = true;
			if (*dm->p_band_width == ODM_BW20M)
	...
	ODM_RT_TRACE(
		...
		("DIG: Link status: is_linked = %d, RSSI = %d, bFirstConnect = %d, bFirsrDisConnect = %d\n",
		 dm->is_linked, dm->rssi_min, first_connect,
		 first_dis_connect));
	...
	ODM_RT_TRACE(dm, ODM_COMP_DIG, ODM_DBG_LOUD,
		     ("DIG: cur_ig_value=0x%x, TotalFA = %d\n", current_igi,
		      fa_cnt->cnt_all));
	...
	ODM_RT_TRACE(dm, ODM_COMP_DIG, ODM_DBG_LOUD, ("DIG end\n"));
}
...
void odm_dig_by_rssi_lps(void *dm_void)
{
	...
	ODM_RT_TRACE(dm, ODM_COMP_DIG, ODM_DBG_LOUD, ("%s()==>\n", __func__));
	...
	ODM_RT_TRACE(
		dm, ODM_COMP_DIG, ODM_DBG_LOUD,
		("%s(): fa_cnt->cnt_all = %d\n", __func__, fa_cnt->cnt_all));
	ODM_RT_TRACE(dm, ODM_COMP_DIG, ODM_DBG_LOUD,
		     ("%s(): dm->rssi_min = %d\n", __func__, dm->rssi_min));
	ODM_RT_TRACE(dm, ODM_COMP_DIG, ODM_DBG_LOUD,
		     ("%s(): current_igi = 0x%x\n", __func__, current_igi));
...
void odm_false_alarm_counter_statistics(void *dm_void)
{
	...
	struct false_alarm_stat *false_alm_cnt =
		(struct false_alarm_stat *)phydm_get_structure(
			dm, PHYDM_FALSEALMCNT);
	struct _RT_ADCSMP *adc_smp = &dm->adcsmp;
	u32 ret_value;

	if (!(dm->support_ability & ODM_BB_FA_CNT))
		return;

	ODM_RT_TRACE(dm, ODM_COMP_FA_CNT, ODM_DBG_LOUD,
		     ("%s()======>\n", __func__));
	...
	if (dm->support_ic_type & ODM_IC_11N_SERIES) {
		...
		ODM_RT_TRACE(
			dm, ODM_COMP_FA_CNT, ODM_DBG_LOUD,
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		if (cck_enable) { /* if(*dm->p_band_type == ODM_BAND_2_4G) */
...
void odm_cck_packet_detection_thresh(void *dm_void)
{
	...
	ODM_RT_TRACE(dm, ODM_COMP_DIG, ODM_DBG_LOUD, ("CCK_PD: ==========>\n"));
	...
	ODM_RT_TRACE(
		dm, ODM_COMP_DIG, ODM_DBG_LOUD,
		("CCK_PD: CCK FA moving average = %d\n", dig_tab->cck_fa_ma));
