...
void phydm_seq_sorting(void *dm_void, u32 *p_value, u32 *rank_idx,
...
void odm_update_mp_driver_status(struct phy_dm_struct *dm) { /* Do nothing. */ }

void phydm_init_trx_antenna_setting(struct phy_dm_struct *dm)
{
...
void phydm_traffic_load_decision(void *dm_void)
{
	...
	if (dm->last_tx_ok_cnt > *dm->p_num_tx_bytes_unicast)
		dm->last_tx_ok_cnt = *dm->p_num_tx_bytes_unicast;

	if (dm->last_rx_ok_cnt > *dm->p_num_rx_bytes_unicast)
		dm->last_rx_ok_cnt = *dm->p_num_rx_bytes_unicast;

	dm->cur_tx_ok_cnt = *dm->p_num_tx_bytes_unicast - dm->last_tx_ok_cnt;
	dm->cur_rx_ok_cnt = *dm->p_num_rx_bytes_unicast - dm->last_rx_ok_cnt;
	dm->last_tx_ok_cnt = *dm->p_num_tx_bytes_unicast;
	dm->last_rx_ok_cnt = *dm->p_num_rx_bytes_unicast;
...
void phydm_config_ofdm_tx_path(struct phy_dm_struct *dm, u32 path) {}
...
void phydm_config_cck_rx_antenna_init(struct phy_dm_struct *dm) {}

void phydm_config_cck_rx_path(struct phy_dm_struct *dm, u8 path, u8 path_div_en)
{
}
...
void phydm_init_cck_setting(struct phy_dm_struct *dm)
{
...
void phydm_dynamicsoftmletting(struct phy_dm_struct *dm)
{
	u32 ret_val;

	if (!dm->mp_mode) {
		if (dm->support_ic_type & ODM_RTL8822B) {
			if ((!dm->is_linked) | (dm->is_linkedcmw500))
				return;

			if (dm->bsomlenabled) {
				ODM_RT_TRACE(
					dm, ODM_COMP_API, ODM_DBG_TRACE,
					("%s(): SoML has been enable, skip dynamic SoML switch\n",
					 __func__));
				return;
			}

			ret_val = odm_get_bb_reg(dm, 0xf8c, MASKBYTE0);
			ODM_RT_TRACE(dm, ODM_COMP_API, ODM_DBG_TRACE,
				     ("%s(): Read 0xF8C = 0x%08X\n", __func__,
				      ret_val));

			if (ret_val < 0x16) {
				ODM_RT_TRACE(
					dm, ODM_COMP_API, ODM_DBG_LOUD,
					("%s(): 0xF8C(== 0x%08X) < 0x16, enable SoML\n",
					 __func__, ret_val));
				odm_set_bb_reg(dm, 0x19a8, MASKDWORD,
					       0xc10a0000);
...
void phydm_init_soft_ml_setting(struct phy_dm_struct *dm)
{
	if (!dm->mp_mode) {
		if (dm->support_ic_type & ODM_RTL8822B)
...
void phydm_init_hw_info_by_rfe(struct phy_dm_struct *dm)
{
	if (dm->support_ic_type & ODM_RTL8822B)
		phydm_init_hw_info_by_rfe_type_8822b(dm);
}

void odm_common_info_self_init(struct phy_dm_struct *dm)
{
...
void odm_common_info_self_update(struct phy_dm_struct *dm)
{
	...
	if (*dm->p_band_width == ODM_BW40M) {
		if (*dm->p_sec_ch_offset == 1)
			dm->control_channel = *dm->p_channel - 2;
		else if (*dm->p_sec_ch_offset == 2)
			dm->control_channel = *dm->p_channel + 2;
	} else {
		dm->control_channel = *dm->p_channel;
	}

	for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
		entry = dm->p_odm_sta_info[i];
...
void odm_common_info_self_reset(struct phy_dm_struct *dm)
{
...
void odm_hw_setting(struct phy_dm_struct *dm)
{
	if (dm->support_ic_type & ODM_RTL8822B)
...
void phydm_supportability_init(void *dm_void)
{
	...
	if (*dm->p_enable_antdiv)
		support_ability |= ODM_BB_ANT_DIV;

	if (*dm->p_enable_adaptivity) {
	...
	if (dm_value[0] == 100) {
		...
		PHYDM_SNPRINTF((output + used, out_len - used,
				...
				((dm->support_ability & ODM_BB_RSSI_MONITOR) ?
					 ...
					 ("."))));
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "05. (( %s ))CCK_PD\n",
		...
		PHYDM_SNPRINTF(
			(output + used, out_len - used,
			 "20. (( %s ))EDCA_TURBO\n",
			 ((dm->support_ability & ODM_MAC_EDCA_TURBO) ? ("V") :
								       ("."))));
		PHYDM_SNPRINTF(
			(output + used, out_len - used,
		...
		PHYDM_SNPRINTF(
			(output + used, out_len - used,
			 "26. (( %s ))RF_CALIBRATION\n",
			 ((dm->support_ability & ODM_RF_CALIBRATION) ? ("V") :
								       ("."))));
		PHYDM_SNPRINTF((output + used, out_len - used, "%s\n",
				"================================"));
	} else {
 ...
 */
void odm_cmn_info_init(struct phy_dm_struct *dm, enum odm_cmninfo_e cmn_info,
		       u32 value)
{
...
void odm_cmn_info_hook(struct phy_dm_struct *dm, enum odm_cmninfo_e cmn_info,
		       void *p_value)
{
	/*  */
	...
	case ODM_CMNINFO_MAC_PHY_MODE:
		dm->p_mac_phy_mode = (u8 *)p_value;
		break;

	case ODM_CMNINFO_TX_UNI:
		dm->p_num_tx_bytes_unicast = (u64 *)p_value;
		break;

	case ODM_CMNINFO_RX_UNI:
		dm->p_num_rx_bytes_unicast = (u64 *)p_value;
		break;

	case ODM_CMNINFO_WM_MODE:
		dm->p_wireless_mode = (u8 *)p_value;
		break;

	case ODM_CMNINFO_BAND:
		dm->p_band_type = (u8 *)p_value;
		break;

	case ODM_CMNINFO_SEC_CHNL_OFFSET:
		dm->p_sec_ch_offset = (u8 *)p_value;
		break;

	case ODM_CMNINFO_SEC_MODE:
		dm->p_security = (u8 *)p_value;
		break;

	case ODM_CMNINFO_BW:
		dm->p_band_width = (u8 *)p_value;
		break;

	case ODM_CMNINFO_CHNL:
		dm->p_channel = (u8 *)p_value;
		break;

	case ODM_CMNINFO_DMSP_GET_VALUE:
		dm->p_is_get_value_from_other_mac = (bool *)p_value;
		break;

	case ODM_CMNINFO_BUDDY_ADAPTOR:
		dm->p_buddy_adapter = (void **)p_value;
		break;

	case ODM_CMNINFO_DMSP_IS_MASTER:
		dm->p_is_master_of_dmsp = (bool *)p_value;
		break;

	case ODM_CMNINFO_SCAN:
		dm->p_is_scan_in_process = (bool *)p_value;
		break;

	case ODM_CMNINFO_POWER_SAVING:
		dm->p_is_power_saving = (bool *)p_value;
		break;

	case ODM_CMNINFO_ONE_PATH_CCA:
		dm->p_one_path_cca = (u8 *)p_value;
		break;

	case ODM_CMNINFO_DRV_STOP:
		dm->p_is_driver_stopped = (bool *)p_value;
		break;

	case ODM_CMNINFO_PNP_IN:
		dm->p_is_driver_is_going_to_pnp_set_power_sleep =
			(bool *)p_value;
		break;

	case ODM_CMNINFO_INIT_ON:
		dm->pinit_adpt_in_progress = (bool *)p_value;
		break;

	case ODM_CMNINFO_ANT_TEST:
		dm->p_antenna_test = (u8 *)p_value;
		break;

	case ODM_CMNINFO_NET_CLOSED:
		dm->p_is_net_closed = (bool *)p_value;
		break;

	case ODM_CMNINFO_FORCED_RATE:
		dm->p_forced_data_rate = (u16 *)p_value;
		break;
	case ODM_CMNINFO_ANT_DIV:
		dm->p_enable_antdiv = (u8 *)p_value;
		break;
	case ODM_CMNINFO_ADAPTIVITY:
		dm->p_enable_adaptivity = (u8 *)p_value;
		break;
	case ODM_CMNINFO_FORCED_IGI_LB:
		dm->pu1_forced_igi_lb = (u8 *)p_value;
		break;

	case ODM_CMNINFO_P2P_LINK:
		dm->dm_dig_table.is_p2p_in_process = (u8 *)p_value;
		break;

	case ODM_CMNINFO_IS1ANTENNA:
		dm->p_is_1_antenna = (bool *)p_value;
		break;

	case ODM_CMNINFO_RFDEFAULTPATH:
		dm->p_rf_default_path = (u8 *)p_value;
		break;

	case ODM_CMNINFO_FCS_MODE:
		dm->p_is_fcs_mode_enable = (bool *)p_value;
		break;
	/*add by YuChen for beamforming PhyDM*/
	case ODM_CMNINFO_HUBUSBMODE:
		dm->hub_usb_mode = (u8 *)p_value;
		break;
	case ODM_CMNINFO_FWDWRSVDPAGEINPROGRESS:
		dm->p_is_fw_dw_rsvd_page_in_progress = (bool *)p_value;
		break;
	case ODM_CMNINFO_TX_TP:
		dm->p_current_tx_tp = (u32 *)p_value;
		break;
	case ODM_CMNINFO_RX_TP:
		dm->p_current_rx_tp = (u32 *)p_value;
		break;
	case ODM_CMNINFO_SOUNDING_SEQ:
		dm->p_sounding_seq = (u8 *)p_value;
		break;
	case ODM_CMNINFO_FORCE_TX_ANT_BY_TXDESC:
		dm->dm_fat_table.p_force_tx_ant_by_desc = (u8 *)p_value;
		break;
	case ODM_CMNINFO_SET_S0S1_DEFAULT_ANTENNA:
		dm->dm_fat_table.p_default_s0_s1 = (u8 *)p_value;
		break;
...
void odm_cmn_info_ptr_array_hook(struct phy_dm_struct *dm,
				 enum odm_cmninfo_e cmn_info, u16 index,
				 void *p_value)
{
	/*Hook call by reference pointer.*/
	switch (cmn_info) {
	...
	case ODM_CMNINFO_STA_STATUS:
		dm->p_odm_sta_info[index] = (struct rtl_sta_info *)p_value;

		if (IS_STA_VALID(dm->p_odm_sta_info[index]))
...
static void odm_update_power_training_state(struct phy_dm_struct *dm)
{
	...
	ODM_RT_TRACE(dm, ODM_COMP_RA_MASK, ODM_DBG_LOUD,
		     ("%s()============>\n", __func__));
...
void phydm_noisy_detection(struct phy_dm_struct *dm)
{
	...
	ODM_RT_TRACE(
		dm, ODM_COMP_NOISY_DETECT, ODM_DBG_LOUD,
...
void phydm_csi_mask_enable(void *dm_void, u32 enable)
{
	...
	if (dm->support_ic_type & ODM_IC_11N_SERIES) {
		...
		ODM_RT_TRACE(dm, ODM_COMP_API, ODM_DBG_LOUD,
		      ...
		      u32 *fc_in)
{
	...
	u32 start_ch_per_40m[NUM_START_CH_40M] = {36,  44,  52,  60,  100,
						  108, 116, 124, 132, 140,
						  149, 157, 165, 173};
	u32 start_ch_per_80m[NUM_START_CH_80M] = {36,  52,  100, 116,
						  132, 149, 165};
...
u8 phydm_calculate_intf_distance(void *dm_void, u32 bw, u32 fc,
				 u32 f_interference, u32 *p_tone_idx_tmp_in)
{
	struct phy_dm_struct *dm = (struct phy_dm_struct *)dm_void;
	u32 bw_up, bw_low;
	u32 int_distance;
