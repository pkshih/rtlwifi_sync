...
#define READ_FIRMWARE_MP(ic, txt)                                              \
	(odm_read_firmware_mp_##ic##txt(dm, p_firmware, p_size))
#define READ_FIRMWARE_TC(ic, txt)                                              \
	(odm_read_firmware_tc_##ic##txt(dm, p_firmware, p_size))
...
u32 phydm_process_rssi_pwdb(struct phy_dm_struct *dm,
			    ...
			    u32 undecorated_smoothed_cck)
{
	...
	if (entry->rssi_stat.ofdm_pkt ==
		...
		ODM_RT_TRACE(dm, ODM_COMP_RSSI_MONITOR, ODM_DBG_LOUD,
			     ("PWDB_0[%d] = (( %d ))\n", pktinfo->station_id,
		...
		if (entry->rssi_stat.valid_bit == 64) {
			...
			ODM_RT_TRACE(
				dm, ODM_COMP_RSSI_MONITOR, ODM_DBG_LOUD,
...
u32 phydm_process_rssi_cck(struct phy_dm_struct *dm,
...
u32 phydm_process_rssi_ofdm(struct phy_dm_struct *dm,
			    struct dm_phy_status_info *phy_info,
			    struct rtl_sta_info *entry,
			    u32 undecorated_smoothed_ofdm)
{
...
{
	if (i < ODM_RF_PATH_C)
		return odm_evm_dbm_jaguar_series(p_phy_sta_rpt->rxevm[i]);
	else
		return odm_evm_dbm_jaguar_series(
			p_phy_sta_rpt->rxevm_cd[i - 2]);
	/*RT_DISP(FRX, RX_PHY_SQ, ("RXRATE=%x RXEVM=%x EVM=%s%d\n",*/
	/*pktinfo->data_rate, p_phy_sta_rpt->rxevm[i], "%", EVM));*/
...
static inline u8 phydm_get_rx_pwr(u8 LNA_idx, u8 VGA_idx, u8 cck_highpwr)
{
	switch (LNA_idx) {
	case 7:
		if (VGA_idx <= 27)
...
u8 odm_query_rx_pwr_percentage(s8 ant_power)
{
 ...
 */
s32 odm_signal_scale_mapping_92c_series_patch_rt_cid_819x_lenovo(
	struct phy_dm_struct *dm, s32 curr_sig)
{
	s32 ret_sig = 0;
...
s32 odm_signal_scale_mapping_92c_series_patch_rt_cid_819x_netcore(
	struct phy_dm_struct *dm, s32 curr_sig)
{
	s32 ret_sig = 0;
...
s32 odm_signal_scale_mapping_92c_series(struct phy_dm_struct *dm, s32 curr_sig)
{
	s32 ret_sig = 0;
...
u8 phydm_rate_to_num_ss(struct phy_dm_struct *dm, u8 data_rate)
{
	u8 num_ss = 1;
...
void odm_rx_phy_status92c_series_parsing(struct phy_dm_struct *dm,
					 struct dm_phy_status_info *phy_info,
					 u8 *p_phy_status,
					 struct dm_per_pkt_info *pktinfo)
{
	...
	if (is_cck_rate) {
		...
		for (i = ODM_RF_PATH_A; i < ODM_RF_PATH_MAX; i++) {
			...
			dm->phy_dbg_info.rx_snr_db[i] =
				(s32)(p_phy_sta_rpt->path_rxsnr[i] / 2);
		...
		odm_parsing_cfo(dm, pktinfo, p_phy_sta_rpt->path_cfotail,
				num_ss);
	}
...
void odm_rx_phy_bw_jaguar_series_parsing(
	struct dm_phy_status_info *phy_info, struct dm_per_pkt_info *pktinfo,
	struct _phy_status_rpt_8812 *p_phy_sta_rpt)
{
...
void odm_rx_phy_status_jaguar_series_parsing(
	struct phy_dm_struct *dm, struct dm_phy_status_info *phy_info,
	u8 *p_phy_status, struct dm_per_pkt_info *pktinfo)
{
	u8 i, max_spatial_stream;
	s8 rx_pwr[4], rx_pwr_all = 0;
	...
	odm_rx_phy_bw_jaguar_series_parsing(phy_info, pktinfo, p_phy_sta_rpt);
	...
	if (is_cck_rate) {
		...
		/*(3) Get Signal Quality (EVM)*/
		{
			...
			if ((dm->support_platform == ODM_WIN) &&
				...
				sq = phydm_get_signal_quality_8812(
					phy_info, dm, p_phy_sta_rpt);
		...
		/*is OFDM rate*/
		fat_tab->hw_antsw_occur = p_phy_sta_rpt->hw_antsw_occur;
		...
		for (i = ODM_RF_PATH_A; i < ODM_RF_PATH_MAX_JAGUAR; i++) {
			...
			if (i < ODM_RF_PATH_C) {
				phy_info->cfo_short[i] =
					odm_cfo((p_phy_sta_rpt->cfosho[i]));
				phy_info->cfo_tail[i] =
					odm_cfo((p_phy_sta_rpt->cfotail[i]));
			}
		}
		...
		if ((dm->support_platform == ODM_WIN) && (dm->patch_id == 19)) {
			...
			for (i = 0; i < max_spatial_stream; i++) {
				...
				EVM = phydm_get_odm_evm(i, pktinfo,
							p_phy_sta_rpt);
				evm_dbm = phydm_get_evm_dbm(
					i, EVM, p_phy_sta_rpt, phy_info);
		...
		num_ss = phydm_rate_to_num_ss(dm, pktinfo->data_rate);
		odm_parsing_cfo(dm, pktinfo, p_phy_sta_rpt->cfotail, num_ss);
	}
	...
	dm->dm_fat_table.antsel_rx_keep_0 = p_phy_sta_rpt->antidx_anta;
	dm->dm_fat_table.antsel_rx_keep_1 = p_phy_sta_rpt->antidx_antb;
	dm->dm_fat_table.antsel_rx_keep_2 = p_phy_sta_rpt->antidx_antc;
	dm->dm_fat_table.antsel_rx_keep_3 = p_phy_sta_rpt->antidx_antd;
}
...
void phydm_reset_rssi_for_dm(struct phy_dm_struct *dm, u8 station_id)
{
	...
	entry = dm->p_odm_sta_info[station_id];
...
void odm_process_rssi_for_dm(struct phy_dm_struct *dm,
			     struct dm_phy_status_info *phy_info,
			     struct dm_per_pkt_info *pktinfo)
{
	...
	entry = dm->p_odm_sta_info[pktinfo->station_id];
 ...
 */
void odm_phy_status_query_92c_series(struct phy_dm_struct *dm,
				     struct dm_phy_status_info *phy_info,
				     u8 *p_phy_status,
				     struct dm_per_pkt_info *pktinfo)
{
	odm_rx_phy_status92c_series_parsing(dm, phy_info, p_phy_status,
					    pktinfo);
...
void odm_phy_status_query_jaguar_series(struct phy_dm_struct *dm,
					struct dm_phy_status_info *phy_info,
					u8 *p_phy_status,
					struct dm_per_pkt_info *pktinfo)
{
	odm_rx_phy_status_jaguar_series_parsing(dm, phy_info, p_phy_status,
						pktinfo);
	odm_process_rssi_for_dm(dm, phy_info, pktinfo);
}

void odm_phy_status_query(struct phy_dm_struct *dm,
			  struct dm_phy_status_info *phy_info, u8 *p_phy_status,
			  struct dm_per_pkt_info *pktinfo)
{
	if (dm->support_ic_type & ODM_IC_PHY_STATUE_NEW_TYPE) {
		phydm_rx_phy_status_new_type(dm, p_phy_status, pktinfo,
					     phy_info);
		return;
	}

	if (dm->support_ic_type & ODM_IC_11AC_SERIES)
		odm_phy_status_query_jaguar_series(dm, phy_info, p_phy_status,
						   pktinfo);

	if (dm->support_ic_type & ODM_IC_11N_SERIES)
		odm_phy_status_query_92c_series(dm, phy_info, p_phy_status,
						pktinfo);
}

/* For future use. */
void odm_mac_status_query(struct phy_dm_struct *dm, u8 *p_mac_status, u8 mac_id,
...
{
	ODM_RT_TRACE(dm, ODM_COMP_INIT, ODM_DBG_LOUD,
		     ("===>odm_config_rf_with_header_file (%s)\n",
		      (dm->is_mp_chip) ? "MPChip" : "TestChip"));
	ODM_RT_TRACE(
		dm, ODM_COMP_INIT, ODM_DBG_LOUD,
...
bool phydm_query_is_mu_api(struct phy_dm_struct *p_phydm, u8 ppdu_idx,
...
void phydm_rx_statistic_cal(struct phy_dm_struct *p_phydm, u8 *p_phy_status,
			    struct dm_per_pkt_info *pktinfo)
{
	...
	if ((p_phy_sta_rpt->gid != 0) && (p_phy_sta_rpt->gid != 63)) {
		if (date_rate >= ODM_RATEVHTSS1MCS0) {
			...
			p_phydm->phy_dbg_info.num_of_ppdu[pktinfo->ppdu_cnt] =
				date_rate | BIT(7);
			p_phydm->phy_dbg_info.gid_num[pktinfo->ppdu_cnt] =
				p_phy_sta_rpt->gid;
		}
		...
		if (date_rate >= ODM_RATEVHTSS1MCS0) {
			p_phydm->phy_dbg_info
				.num_qry_vht_pkt[date_rate - 0x2C]++;
			p_phydm->phy_dbg_info.num_of_ppdu[pktinfo->ppdu_cnt] =
				date_rate;
			p_phydm->phy_dbg_info.gid_num[pktinfo->ppdu_cnt] =
				p_phy_sta_rpt->gid;
			  ...
			  struct dm_phy_status_info *phy_info)
{
	...
	phy_info->band_width = 0;
	phy_info->rx_count = 0;
	odm_memory_set(p_phydm, phy_info->rx_mimo_signal_quality, 0, 4);
	odm_memory_set(p_phydm, phy_info->rx_mimo_signal_strength, 0, 4);
	odm_memory_set(p_phydm, phy_info->rx_snr, 0, 4);
	...
	phy_info->rxsc = 0;
	odm_memory_set(p_phydm, phy_info->rx_pwr, -110, 4);
	odm_memory_set(p_phydm, phy_info->rx_mimo_evm_dbm, 0, 4);
	odm_memory_set(p_phydm, phy_info->cfo_short, 0, 8);
	odm_memory_set(p_phydm, phy_info->cfo_tail, 0, 8);
}

void phydm_set_per_path_phy_info(u8 rx_path, s8 rx_pwr, s8 rx_evm, s8 cfo_tail,
				 s8 rx_snr, struct dm_phy_status_info *phy_info)
{
	u8 evm_dbm = 0;
	u8 evm_percentage = 0;
...
void phydm_set_common_phy_info(s8 rx_power, u8 channel, bool is_beamformed,
			       bool is_mu_packet, u8 bandwidth,
...
void phydm_get_rx_phy_status_type0(struct phy_dm_struct *dm, u8 *p_phy_status,
				   struct dm_per_pkt_info *pktinfo,
				   struct dm_phy_status_info *phy_info)
{
	...
	phydm_set_common_phy_info(rx_power, p_phy_sta_rpt->channel, false,
				  false, ODM_BW20M, sq, p_phy_sta_rpt->rxsc,
				  phy_info);
	...
	dm->dm_fat_table.antsel_rx_keep_0 = p_phy_sta_rpt->antidx_a;
	dm->dm_fat_table.antsel_rx_keep_1 = p_phy_sta_rpt->antidx_b;
	dm->dm_fat_table.antsel_rx_keep_2 = p_phy_sta_rpt->antidx_c;
	dm->dm_fat_table.antsel_rx_keep_3 = p_phy_sta_rpt->antidx_d;
}

void phydm_get_rx_phy_status_type1(struct phy_dm_struct *dm, u8 *p_phy_status,
				   struct dm_per_pkt_info *pktinfo,
				   struct dm_phy_status_info *phy_info)
{
	...
	for (i = ODM_RF_PATH_A; i < ODM_RF_PATH_MAX_JAGUAR; i++) {
		if (dm->rx_ant_status & BIT(i)) {
			...
			phydm_set_per_path_phy_info(
				...
				p_phy_sta_rpt->rxsnr[i], phy_info);
	...
	dm->phy_dbg_info.num_qry_bf_pkt =
		dm->phy_dbg_info.num_qry_bf_pkt + p_phy_sta_rpt->beamformed;

	/*STBC or LDPC pkt*/
	dm->phy_dbg_info.is_ldpc_pkt = p_phy_sta_rpt->ldpc;
	dm->phy_dbg_info.is_stbc_pkt = p_phy_sta_rpt->stbc;
	...
	phydm_set_common_phy_info(
		rx_pwr_db, p_phy_sta_rpt->channel,
		(bool)p_phy_sta_rpt->beamformed, is_mu, bw,
		odm_evm_db_to_percentage(p_phy_sta_rpt->rxevm[0]), rxsc,
		phy_info);

	num_ss = phydm_rate_to_num_ss(dm, pktinfo->data_rate);

	odm_parsing_cfo(dm, pktinfo, p_phy_sta_rpt->cfo_tail, num_ss);
	dm->dm_fat_table.antsel_rx_keep_0 = p_phy_sta_rpt->antidx_a;
	dm->dm_fat_table.antsel_rx_keep_1 = p_phy_sta_rpt->antidx_b;
	dm->dm_fat_table.antsel_rx_keep_2 = p_phy_sta_rpt->antidx_c;
	dm->dm_fat_table.antsel_rx_keep_3 = p_phy_sta_rpt->antidx_d;
	...
	if (pktinfo->is_packet_match_bssid) {
		/* */
		phydm_rx_statistic_cal(dm, p_phy_status, pktinfo);
	}
}

void phydm_get_rx_phy_status_type2(struct phy_dm_struct *dm, u8 *p_phy_status,
				   struct dm_per_pkt_info *pktinfo,
				   struct dm_phy_status_info *phy_info)
{
	...
	phydm_set_common_phy_info(rx_pwr_db, p_phy_sta_rpt->channel,
				  (bool)p_phy_sta_rpt->beamformed, false, bw, 0,
				  rxsc, phy_info);
}

void phydm_get_rx_phy_status_type5(u8 *p_phy_status) {}

void phydm_process_rssi_for_dm_new_type(struct phy_dm_struct *dm,
					struct dm_phy_status_info *phy_info,
					struct dm_per_pkt_info *pktinfo)
{
...
void phydm_rx_phy_status_new_type(struct phy_dm_struct *p_phydm,
				  u8 *p_phy_status,
				  struct dm_per_pkt_info *pktinfo,
				  struct dm_phy_status_info *phy_info)
{
	...
	/* Memory reset */
	phydm_reset_phy_info(p_phydm, phy_info);

	/* Phy status parsing */
	switch (phy_status_type) {
	case 0: {
		phydm_get_rx_phy_status_type0(p_phydm, p_phy_status, pktinfo,
					      phy_info);
		break;
	}
	case 1: {
		phydm_get_rx_phy_status_type1(p_phydm, p_phy_status, pktinfo,
					      phy_info);
		break;
	}
	case 2: {
		phydm_get_rx_phy_status_type2(p_phydm, p_phy_status, pktinfo,
					      phy_info);
		break;
	}
	default:
	 ...
	 */
	phy_info->signal_strength =
		(u8)(odm_signal_scale_mapping(p_phydm, phy_info->rx_pwdb_all));

	/* Calculate average RSSI and smoothed RSSI */
	phydm_process_rssi_for_dm_new_type(p_phydm, phy_info, pktinfo);
}
