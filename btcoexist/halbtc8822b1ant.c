...
u32 glcoex_ver_date_8822b_1ant = 2016116;
u32 glcoex_ver_8822b_1ant = 0x3d;
...
u8 halbtc8822b1ant_bt_rssi_state(u8 level_num, u8 rssi_thresh, u8 rssi_thresh1)
{
	...
	if (level_num == 2) {
		...
		if (rssi_thresh > rssi_thresh1) {
			BTC_PRINTF("[BTCoex], BT Rssi thresh error!!\n");
...
u8 halbtc8822b1ant_wifi_rssi_state(struct btc_coexist *btcoexist, u8 index,
				   ...
				   u8 rssi_thresh1)
{
	...
	if (level_num == 2) {
		...
		if (rssi_thresh > rssi_thresh1) {
			BTC_PRINTF("[BTCoex], wifi RSSI thresh error!!\n");
...
void halbtc8822b1ant_update_ra_mask(struct btc_coexist *btcoexist,
...
void halbtc8822b1ant_auto_rate_fallback_retry(struct btc_coexist *btcoexist,
...
void halbtc8822b1ant_retry_limit(struct btc_coexist *btcoexist, bool force_exec,
				 u8 type)
{
...
void halbtc8822b1ant_ampdu_max_time(struct btc_coexist *btcoexist,
				    bool force_exec, u8 type)
{
...
void halbtc8822b1ant_limited_tx(struct btc_coexist *btcoexist, bool force_exec,
...
void halbtc8822b1ant_limited_rx(struct btc_coexist *btcoexist, bool force_exec,
...
void halbtc8822b1ant_query_bt_info(struct btc_coexist *btcoexist)
{
...
void halbtc8822b1ant_monitor_bt_ctr(struct btc_coexist *btcoexist)
{
	...
	BTC_PRINTF("[BTCoex], Hi-Pri Rx/Tx: %d/%d, Lo-Pri Rx/Tx: %d/%d\n",
...
void halbtc8822b1ant_monitor_wifi_ctr(struct btc_coexist *btcoexist)
{
	s32 wifi_rssi = 0;
...
bool halbtc8822b1ant_is_wifi_status_changed(struct btc_coexist *btcoexist)
{
	static bool pre_wifi_busy = false, pre_under_4way = false,
		    pre_bt_hs_on = false, pre_rf4ce_enabled = false,
		    pre_bt_off = false;
	...
	if (coex_sta->bt_disabled != pre_bt_off) {
		...
		if (coex_sta->bt_disabled)
			BTC_PRINTF("[BTCoex], BT is disabled !!\n");
		else
			BTC_PRINTF("[BTCoex], BT is enabled !!\n");
	...
	if (rf4ce_enabled != pre_rf4ce_enabled) {
		...
		if (rf4ce_enabled)
			BTC_PRINTF("[BTCoex], rf4ce is enabled !!\n");
		else
			BTC_PRINTF("[BTCoex], rf4ce is disabled !!\n");
...
void halbtc8822b1ant_update_bt_link_info(struct btc_coexist *btcoexist)
{
	struct btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
	bool bt_hs_on = false;
...
void halbtc8822b1ant_update_wifi_channel_info(struct btc_coexist *btcoexist,
					      u8 type)
{
	...
	if ((type == BTC_MEDIA_CONNECT) && (wifi_central_chnl <= 14)) {
		/* enable BT AFH skip WL channel for 8822b because BT Rx LO
		 * interference
		 */
		h2c_parameter[0] = 0x1;
...
u8 halbtc8822b1ant_action_algorithm(struct btc_coexist *btcoexist)
{
	struct btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
	bool bt_hs_on = false;
	...
	if (!bt_link_info->bt_link_exist) {
		BTC_PRINTF("[BTCoex], No BT link exists!!!\n");
		return algorithm;
	}
	...
	if (num_of_diff_profile == 1) {
		if (bt_link_info->sco_exist) {
			BTC_PRINTF("[BTCoex], BT Profile = SCO only\n");
			algorithm = BT_8822B_1ANT_COEX_ALGO_SCO;
		} else {
			if (bt_link_info->hid_exist) {
				BTC_PRINTF("[BTCoex], BT Profile = HID only\n");
...
void halbtc8822b1ant_set_bt_auto_report(struct btc_coexist *btcoexist,
...
void halbtc8822b1ant_bt_auto_report(struct btc_coexist *btcoexist,
...
void halbtc8822b1ant_set_sw_penalty_tx_rate_adaptive(
...
void halbtc8822b1ant_low_penalty_ra(struct btc_coexist *btcoexist,
...
void halbtc8822b1ant_write_score_board(struct btc_coexist *btcoexist,
...
void halbtc8822b1ant_read_score_board(struct btc_coexist *btcoexist,
				      u16 *score_board_val)
{
	*score_board_val =
...
void halbtc8822b1ant_post_state_to_bt(struct btc_coexist *btcoexist, u16 type,
				      bool state)
{
	halbtc8822b1ant_write_score_board(btcoexist, (u16)type, state);
}

void halbtc8822b1ant_monitor_bt_enable_disable(struct btc_coexist *btcoexist)
{
	...
	if (coex_sta->bt_disabled != bt_disabled) {
		BTC_PRINTF("[BTCoex], BT is from %s to %s!!\n",
...
void halbtc8822b1ant_enable_gnt_to_gpio(struct btc_coexist *btcoexist,
					bool isenable)
{
...
u32 halbtc8822b1ant_ltecoex_indirect_read_reg(struct btc_coexist *btcoexist,
					      u16 reg_addr)
{
	u32 j = 0;
...
void halbtc8822b1ant_ltecoex_indirect_write_reg(struct btc_coexist *btcoexist,
...
void halbtc8822b1ant_ltecoex_enable(struct btc_coexist *btcoexist, bool enable)
{
	u8 val;
...
void halbtc8822b1ant_ltecoex_pathcontrol_owner(struct btc_coexist *btcoexist,
					       bool wifi_control)
{
	u8 val;
...
void halbtc8822b1ant_ltecoex_set_gnt_bt(struct btc_coexist *btcoexist,
					u8 control_block, bool sw_control,
					u8 state)
{
	u32 val = 0, bit_mask;
...
void halbtc8822b1ant_ltecoex_set_gnt_wl(struct btc_coexist *btcoexist,
					u8 control_block, bool sw_control,
					u8 state)
{
	u32 val = 0, bit_mask;
...
void halbtc8822b1ant_ltecoex_set_coex_table(struct btc_coexist *btcoexist,
...
void halbtc8822b1ant_ltcoex_set_break_table(struct btc_coexist *btcoexist,
					    u8 table_type, u8 table_content)
{
	...
	if (reg_addr != 0x0000)
		halbtc8822b1ant_ltecoex_indirect_write_reg(
			btcoexist, reg_addr, 0xff,
			table_content); /* 0xa8[15:0] or 0xb4[15:0] */
}

void halbtc8822b1ant_set_wltoggle_coex_table(struct btc_coexist *btcoexist,
...
void halbtc8822b1ant_coex_table(struct btc_coexist *btcoexist, bool force_exec,
...
void halbtc8822b1ant_coex_table_with_type(struct btc_coexist *btcoexist,
					  bool force_exec, u8 type)
{
	u32 break_table;
	u8 select_table;
...
void halbtc8822b1ant_power_save_state(struct btc_coexist *btcoexist, u8 ps_type,
				      u8 lps_val, u8 rpwm_val)
{
				   ...
				   u8 byte2, u8 byte3, u8 byte4, u8 byte5)
{
	...
	if (ap_enable) {
		if (byte1 & BIT(4) && !(byte1 & BIT(5))) {
			BTC_PRINTF("[BTCoex], FW for 1Ant AP mode\n");
			...
			halbtc8822b1ant_power_save_state(
			     ...
			     bool turn_on, u8 type)
{
	...
	static u8 ps_tdma_byte4_modify = 0x0, pre_ps_tdma_byte4_modify = 0x0;
	...
	if (coex_dm->cur_ps_tdma_on) {
		BTC_PRINTF("[BTCoex], ********TDMA(on, %d) **********\n",
			   coex_dm->cur_ps_tdma);
	} else {
		BTC_PRINTF("[BTCoex], **********TDMA(off, %d) **********\n",
			   coex_dm->cur_ps_tdma);
	}
...
void halbtc8822b1ant_sw_mechanism(struct btc_coexist *btcoexist,
				  bool low_penalty_ra)
{
	halbtc8822b1ant_low_penalty_ra(btcoexist, NORMAL_EXEC, low_penalty_ra);
}
/*rf4 type by efuse , and for ant at main aux inverse use , because is 2x2,
 *and control types are the same ,does not need
 */

void halbtc8822b1ant_set_rfe_type(struct btc_coexist *btcoexist)
{
...
void halbtc8822b1ant_set_ext_ant_switch(struct btc_coexist *btcoexist,
					...
					u8 pos_type)
{
	...
	/*  Normal switch polarity for SPDT, 0xcbd[1:0] = 2b'01 => Ant to BTG,
	...
	if (rfe_type->ext_ant_switch_type ==
		...
		case BT_8822B_1ANT_EXT_ANT_SWITCH_CTRL_BY_BBSW:
			btcoexist->btc_write_1byte_bitmask(
				btcoexist, 0x4e, 0x80, 0x0); /*  0x4c[23] = 0 */
			btcoexist->btc_write_1byte_bitmask(
				btcoexist, 0x4f, 0x01, 0x1); /* 0x4c[24] = 1 */
			/* BB SW, DPDT use RFE_ctrl8 and RFE_ctrl9 as
			 * conctrol pin
			 */
			 ...
			 */
			regval_0xcbd =
				(switch_polatiry_inverse == false ? 0x1 : 0x2);
		...
		case BT_8822B_1ANT_EXT_ANT_SWITCH_CTRL_BY_PTA:
			 ...
			 */
			regval_0xcbd =
				(switch_polatiry_inverse == false ? 0x2 : 0x1);
			btcoexist->btc_write_1byte_bitmask(btcoexist, 0xcbd,
							   0x3, regval_0xcbd);
		...
		case BT_8822B_1ANT_EXT_ANT_SWITCH_CTRL_BY_ANTDIV:
			...
			/* no regval_0xcbd setup required, because antenna
			 * switch control value by antenna diversity
			 */

			break;
		case BT_8822B_1ANT_EXT_ANT_SWITCH_CTRL_BY_MAC:
			btcoexist->btc_write_1byte_bitmask(
				btcoexist, 0x4e, 0x80, 0x1); /*  0x4c[23] = 1 */

			/* 0x64[0] = 1b'0 for no switch_polatiry_inverse,
			 * DPDT_SEL_N =1, DPDT_SEL_P =0
			 */
			regval_0x64 =
				(switch_polatiry_inverse == false ? 0x0 : 0x1);
		...
		case BT_8822B_1ANT_EXT_ANT_SWITCH_CTRL_BY_BT:
			btcoexist->btc_write_1byte_bitmask(
				btcoexist, 0x4e, 0x80, 0x0); /* 0x4c[23] = 0 */
			btcoexist->btc_write_1byte_bitmask(
				btcoexist, 0x4f, 0x01, 0x0); /* 0x4c[24] = 0 */
...
/*set gnt_wl gnt_bt control by sw high low , or hwpta while in power on,
 *ini,wlan off,wlan only ,wl2g non-currrent ,wl2g current,wl5g
 */
...
void halbtc8822b1ant_coex_all_off(struct btc_coexist *btcoexist)
{
	/* sw all off */
...
bool halbtc8822b1ant_is_common_action(struct btc_coexist *btcoexist)
{
	...
	if (!wifi_connected &&
	    BT_8822B_1ANT_BT_STATUS_NON_CONNECTED_IDLE == coex_dm->bt_status) {
		BTC_PRINTF(
		...
		common = true;
				      ...
				      coex_dm->bt_status)) {
		BTC_PRINTF("[BTCoex], Wifi connected + BT connected-idle!!\n");
		...
		common = true;
				       ...
				       coex_dm->bt_status)) {
		BTC_PRINTF("[BTCoex], Wifi non connected-idle + BT Busy!!\n");
...
void halbtc8822b1ant_action_wifi_under5g(struct btc_coexist *btcoexist)
{
	BTC_PRINTF("[BTCoex], under 5g start\n");
...
void halbtc8822b1ant_action_wifi_only(struct btc_coexist *btcoexist)
{
...
/*
void halbtc8822b1ant_action_sco(struct btc_coexist* btcoexist)
{
	halbtc8822b1ant_sw_mechanism(btcoexist, true);
}

void halbtc8822b1ant_action_hid(struct btc_coexist* btcoexist)
{
	halbtc8822b1ant_sw_mechanism(btcoexist, true);
}

void halbtc8822b1ant_action_a2dp(struct btc_coexist* btcoexist)
{
	halbtc8822b1ant_sw_mechanism(btcoexist, false);
}

void halbtc8822b1ant_action_a2dp_pan_hs(struct btc_coexist* btcoexist)
{
	halbtc8822b1ant_sw_mechanism(btcoexist, false);
}

void halbtc8822b1ant_action_pan_edr(struct btc_coexist* btcoexist)
{
	halbtc8822b1ant_sw_mechanism(btcoexist, false);
}

void halbtc8822b1ant_action_pan_hs(struct btc_coexist* btcoexist)
{
	halbtc8822b1ant_sw_mechanism(btcoexist, false);
}

void halbtc8822b1ant_action_pan_edr_a2dp(struct btc_coexist* btcoexist)
{
	halbtc8822b1ant_sw_mechanism(btcoexist, false);
}

void halbtc8822b1ant_action_pan_edr_hid(struct btc_coexist* btcoexist)
{
	halbtc8822b1ant_sw_mechanism(btcoexist, true);
}

void halbtc8822b1ant_action_hid_a2dp_pan_edr(struct btc_coexist* btcoexist)
{
	halbtc8822b1ant_sw_mechanism(btcoexist, true);
}

void halbtc8822b1ant_action_hid_a2dp(struct btc_coexist* btcoexist)
{
	halbtc8822b1ant_sw_mechanism(btcoexist, true);
}
 ...
 * *********************************************
 */
void halbtc8822b1ant_action_bt_whck_test(struct btc_coexist *btcoexist)
{
	BTC_PRINTF("[BTCoex],action_bt_whck_test\n");
...
{
	BTC_PRINTF("[BTCoex],action_wifi_multi_port\n");
	...
	halbtc8822b1ant_coex_table_with_type(btcoexist, NORMAL_EXEC, 17);
}

void halbtc8822b1ant_action_hs(struct btc_coexist *btcoexist)
{
	BTC_PRINTF("[BTCoex], action_hs\n");
...
void halbtc8822b1ant_action_bt_inquiry(struct btc_coexist *btcoexist)
{
	struct btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
...
void halbtc8822b1ant_action_bt_sco_hid_only_busy(struct btc_coexist *btcoexist,
						 u8 wifi_status)
{
	struct btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
	...
	BTC_PRINTF("[BTCoex], ********** (bt sco hid only busy) **********\n");
...
void halbtc8822b1ant_action_wifi_connected_bt_acl_busy(
	struct btc_coexist *btcoexist, u8 wifi_status)
{
	...
	BTC_PRINTF("[BTCoex], ********** (wifi connect acl busy) **********\n");

	if (bt_link_info
		    ->hid_only) { /* HID  + wifi connected idle or busy / 0x778=1@wifi slot */
		...
		bt_link_info
			->a2dp_only) { /* A2DP  + wifi connected idle or busy */
...
*/
		...
		(bt_link_info->hid_exist && bt_link_info->a2dp_exist &&
		 bt_link_info
			 ->pan_exist)) { /* A2DP+PAN(OPP,FTP), HID+A2DP+PAN(OPP,FTP) */
		...
		if (!wifi_busy) {
			...
			halbtc8822b1ant_set_wltoggle_coex_table(
				btcoexist, NORMAL_EXEC, 0x3, 0xaa, 0x5a, 0xaa,
				0xaa);
			halbtc8822b1ant_ps_tdma(btcoexist, NORMAL_EXEC, true,
						49);
		} else {
			...
			halbtc8822b1ant_coex_table_with_type(btcoexist,
							     NORMAL_EXEC, 1);
			halbtc8822b1ant_set_wltoggle_coex_table(
				btcoexist, NORMAL_EXEC, 0x2, 0xaa, 0x5a, 0xaa,
				0xaa);
			halbtc8822b1ant_ps_tdma(btcoexist, NORMAL_EXEC, true,
						49);
		}
		...
		*/
		...
		/*
		if (wifi_status == BT_8723D_1ANT_WIFI_STATUS_CONNECTED_IDLE)
...
void halbtc8822b1ant_action_wifi_not_connected(struct btc_coexist *btcoexist)
{
	...
	BTC_PRINTF("[BTCoex], ********** (wifi not connect) **********\n");
...
void halbtc8822b1ant_action_wifi_not_connected_scan(
	struct btc_coexist *btcoexist)
{
	...
	BTC_PRINTF("[BTCoex], ********** (wifi non connect scan) **********\n");
...
void halbtc8822b1ant_action_wifi_connected_scan(struct btc_coexist *btcoexist)
{
	struct btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
	bool bt_hs_on = false;
	u32 wifi_link_status = 0;
	...
	BTC_PRINTF("[BTCoex], ********** (wifi connect scan) **********\n");
...
void halbtc8822b1ant_action_wifi_connected(struct btc_coexist *btcoexist)
{
	...
	BTC_PRINTF("[BTCoex], CoexForWifiConnect()===>\n");
...
void halbtc8822b1ant_run_sw_coexist_mechanism(struct btc_coexist *btcoexist)
{
	...
	BTC_PRINTF("[BTCoex], ********** (runswcoexmech) **********\n");
	...
	if (halbtc8822b1ant_is_common_action(btcoexist)) {
		...
		case BT_8822B_1ANT_COEX_ALGO_HID_A2DP:
			BTC_PRINTF("[BTCoex], Action algorithm = HID+A2DP.\n");
...
void halbtc8822b1ant_run_coexist_mechanism(struct btc_coexist *btcoexist)
{
	struct btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
	...
	BTC_PRINTF("[BTCoex], RunCoexistMechanism()===>\n");
	...
	if (coex_sta->under_ips) {
		BTC_PRINTF("[BTCoex], wifi is under IPS !!!\n");
		return;
	}

	if (!coex_sta->run_time_state) {
		BTC_PRINTF("[BTCoex], return for run_time_state = false !!!\n");
		return;
	}
	...
	if (wifi_under_5g) {
		...
		BTC_PRINTF("[BTCoex], WiFi is under 5G!!!\n");
		return;
	}

	BTC_PRINTF("[BTCoex], WiFi is under 2G!!!\n");

	halbtc8822b1ant_set_ant_path(btcoexist, BTC_ANT_PATH_AUTO, NORMAL_EXEC,
				     BT_8822B_1ANT_PHASE_2G_RUNTIME);

	if (coex_sta->bt_whck_test) {
		BTC_PRINTF("[BTCoex], BT is under WHCK TEST!!!\n");
	...
	if ((bt_link_info->bt_link_exist) && (wifi_connected)) {
		...
		if (iot_peer != BTC_IOT_PEER_CISCO) {
			if (bt_link_info
				    ->sco_exist) /* if (bt_link_info->bt_hi_pri_link_exist) */
	...
	btcoexist->btc_get(btcoexist, BTC_GET_BL_HS_OPERATION, &bt_hs_on);
	if (coex_sta->c2h_bt_inquiry_page) {
		BTC_PRINTF("############# [BTCoex],  BT Is Inquirying\n");
		halbtc8822b1ant_action_bt_inquiry(btcoexist);
		return;
	} else if (bt_hs_on) {
		halbtc8822b1ant_action_hs(btcoexist);
	...
	if (!wifi_connected) {
		...
		BTC_PRINTF("[BTCoex], wifi is non connected-idle !!!\n");
...
u32 halbtc8822b1ant_psd_log2base(struct btc_coexist *btcoexist, u32 val)
{
	u8 j;
...
void halbtc8822b1ant_init_coex_dm(struct btc_coexist *btcoexist)
{
...
void halbtc8822b1ant_init_hw_config(struct btc_coexist *btcoexist, bool back_up,
				    bool wifi_only)
{
	u8 u8tmp = 0;
	u32 u32tmp1 = 0, u32tmp2 = 0, u32tmp3 = 0;
	...
	BTC_PRINTF("[BTCoex], 1Ant Init HW Config!!\n");
...
void halbtc8822b1ant_init_hw_config_without_bt(struct btc_coexist *btcoexist,
...
void halbtc8822b1ant_psd_showdata(struct btc_coexist *btcoexist)
{
...
void halbtc8822b1ant_psd_max_holddata(struct btc_coexist *btcoexist,
				      u32 gen_count)
{
...
u32 halbtc8822b1ant_psd_getdata(struct btc_coexist *btcoexist, u32 point)
{
...
void halbtc8822b1ant_psd_sweep_point(struct btc_coexist *btcoexist,
				     u32 cent_freq, s32 offset, u32 span,
				     u32 points, u32 avgnum)
{
	u32 i, val, n, k = 0;
...
void ex_halbtc8822b1ant_power_on_setting(struct btc_coexist *btcoexist)
{
	...
	BTC_PRINTF("Ant Det Finish = %s, Ant Det Number  = %d\n",
...
void ex_halbtc8822b1ant_power_on_setting_without_bt(
	struct btc_coexist *btcoexist)
{
	struct btc_board_info *board_info = &btcoexist->board_info;
	u8 u8tmp = 0x0;
	...
	BTC_PRINTF(" 8822b 1-Ant PowerOn Setting without bt\n");
...
void ex_halbtc8822b1ant_init_hw_config(struct btc_coexist *btcoexist,
				       bool wifi_only)
{
	BTC_PRINTF("[BTCoex], ********** (ini hw config) **********\n");

	halbtc8822b1ant_init_hw_config(btcoexist, true, wifi_only);
	btcoexist->stop_coex_dm = false;
}

void ex_halbtc8822b1ant_init_hw_config_without_bt(struct btc_coexist *btcoexist,
						  bool wifi_only)
{
	BTC_PRINTF("[BTCoex], ********** (ini hw config) **********\n");
...
void ex_halbtc8822b1ant_antenna_switch_without_bt(struct btc_coexist *btcoexist)
{
	bool wifi_under_5g = false;
...
{
	BTC_PRINTF("[BTCoex], Coex Mechanism Init!!\n");
...
void ex_halbtc8822b1ant_display_coex_info(struct btc_coexist *btcoexist)
{
	...
	BTC_PRINTF("[BTCoex], ********** (display coexinfo) **********\n");
	BTC_PRINTF(
	...
	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = 0x%x/ 0x%lx/ 0x%x",
		   ...
		   (u32tmp[0] & (BIT(24) | BIT(23))) >> 23, u8tmp[1] & 0x1);
	CL_PRINTF(cli_buf);
	...
	BTC_PRINTF(
		...
		btcoexist->btc_read_1byte(btcoexist, 0xcbd));
	/*
	BTC_PRINTF("[BTCoex], ********** pnp_wake_cnt = %d\n", pnp_wake_cnt);
	*/
}
...
void ex_halbtc8822b1ant_ips_notify(struct btc_coexist *btcoexist, u8 type)
{
	...
	if (type == BTC_IPS_ENTER) {
		BTC_PRINTF("[BTCoex], IPS ENTER notify\n");
		...
		BTC_PRINTF("[BTCoex], IPS LEAVE notify\n");
		halbtc8822b1ant_post_state_to_bt(
...
void ex_halbtc8822b1ant_lps_notify(struct btc_coexist *btcoexist, u8 type)
{
	...
	if (type == BTC_LPS_ENABLE) {
		BTC_PRINTF("[BTCoex], LPS ENABLE notify\n");
		...
		BTC_PRINTF("[BTCoex], LPS DISABLE notify\n");
...
void ex_halbtc8822b1ant_switchband_notify(struct btc_coexist *btcoexist,
					  u8 type)
{
	BTC_PRINTF("[BTCoex], ********** (switchband_notify) **********\n");
...
void ex_halbtc8822b1ant_connect_notify(struct btc_coexist *btcoexist, u8 type)
{
	...
	BTC_PRINTF("[BTCoex], ********** (connect notify) **********\n");
	...
	if (type == BTC_ASSOCIATE_START) {
		BTC_PRINTF("[BTCoex], 2G CONNECT START notify\n");
		...
		BTC_PRINTF("[BTCoex], 2G CONNECT Finish notify\n");
		coex_sta->wifi_is_high_pri_task = false;
					    ...
					    u8 type)
{
	...
	if (type == BTC_MEDIA_CONNECT) {
		BTC_PRINTF("[BTCoex], 2g media connect notify");

		halbtc8822b1ant_post_state_to_bt(
			btcoexist, BT_8822B_1ANT_SCOREBOARD_ACTIVE, true);

		if (wifi_under_5g) {
			BTC_PRINTF("[BTCoex], 5g media notify\n");
		...
		BTC_PRINTF("[BTCoex], 2g media disconnect notify\n");
		coex_dm->arp_cnt = 0;
					       ...
					       u8 type)
{
	...
	btcoexist->btc_get(btcoexist, BTC_GET_BL_WIFI_UNDER_5G, &wifi_under_5g);
	if (wifi_under_5g) {
		BTC_PRINTF("[BTCoex], 5g special packet notify\n");
	...
	if (under_4way) {
		BTC_PRINTF("[BTCoex], specific Packet ---- under_4way!!\n");
				       ...
				       u8 *tmp_buf, u8 length)
{
	...
	BTC_PRINTF("[BTCoex], ********** (BtInfo Notify) **********\n");
	...
	BTC_PRINTF("[BTCoex], Bt info[%d], length=%d, hex data=[", rsp_source,
		   length);
...
{
	BTC_PRINTF("[BTCoex], RF Status notify\n");

	if (type == BTC_RF_ON) {
		BTC_PRINTF("[BTCoex], RF is turned ON!!\n");
		btcoexist->stop_coex_dm = false;
		...
		BTC_PRINTF("[BTCoex], RF is turned OFF!!\n");
...
void ex_halbtc8822b1ant_pnp_notify(struct btc_coexist *btcoexist, u8 pnp_state)
{
	...
	BTC_PRINTF("[BTCoex], Pnp notify\n");

	btcoexist->btc_get(btcoexist, BTC_GET_BL_WIFI_UNDER_5G, &wifi_under_5g);
	if (pnp_state == BTC_WIFI_PNP_SLEEP) {
		BTC_PRINTF("[BTCoex], Pnp notify to SLEEP\n");
		...
		*/
		...
		/*pnp_wake_cnt++;*/
		BTC_PRINTF("[BTCoex], Pnp notify to WAKE UP\n");
